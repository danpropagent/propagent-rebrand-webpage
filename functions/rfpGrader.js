...
// ============================================================
// LIVE PROGRESS + DAILY CAP (Firestore)
// The homepage watches gradeRuns/{runId} while this request is
// still open; progress writes must never break grading.
// ============================================================

const MAX_RUNS_PER_DAY = 50;

// Client-generated run ids: unguessable, doc-id safe.
const isValidRunId = (id) =>
  typeof id === "string" && /^[A-Za-z0-9_-]{10,64}$/.test(id);

/**
 * Build a reporter that merges partial progress into gradeRuns/{runId}.
 * Returns null when no (valid) runId was supplied, so all call sites
 * can guard with `reporter &&`.
 * @param {string|undefined} runId - Client-generated run id
 * @return {Object|null} Reporter with an async report(patch) method
 */
  ...
  return {
    ...
    report: async (patch) => {
      try {
        ...
        logger.error(`Progress write failed for run ${runId}:`, err.message);
      }
    },
  };
};
...
/**
 * Enforce the shared daily grading cap (fail closed).
 * @return {Promise<void>} Rejects with {code: "over_capacity"} when spent
 */
const assertDailyCap = async () => {
  ...
  await getDb().runTransaction(async (tx) => {
    ...
    if (count >= MAX_RUNS_PER_DAY) {
      const err = new Error("Daily grading capacity reached");
      err.code = "over_capacity";
      throw err;
    }
    tx.set(doc, {count: count + 1}, {merge: true});
  });
};

// First sentence of a reasoning paragraph â€” the on-page one-liner.
// (Full analysis ships only in the emailed PDF.)
const firstSentence = (text) => {
  if (!text || typeof text !== "string") return "";
  const match = text.match(/^.*?[.!?](?:\s|$)/);
  return (match ? match[0] : text).trim().slice(0, 220);
};
...
GRADING SCALE:
A (Excellent): The RFP outlines the scope of work in unambiguous, thorough detail. It clearly enumerates each major task and sub-task the contractor must perform, along with specific deliverables for each (e.g. reports, drawings, constructed assets, training sessions, etc.), and criteria for acceptance or quality where applicable. The language is specific (quantities, standards, locations, etc. are given) so bidders know exactly what to provide. Scope boundaries are defined (what is within project scope and what is not). Example Excerpt: â€œScope of Work: The contractor shall demolish the existing 2-story annex building (~5,000 sq ft) and construct a new 10,000 sq ft library wing. Key deliverables include: (1) Demolition plan â€“ submit 2 weeks prior to demo for approval; (2) Monthly progress reports; (3) As-built drawings and a commissioning report upon completion. Precision: All new construction must conform to the attached 30% design drawings and specifications Section 01â€“20. The RFP provides a task breakdown (design, permitting, construction, commissioning) with milestones for each. Acceptance Criteria: e.g., â€œCityâ€™s approval of the commissioning report and a post-construction inspection with zero safety violationsâ€ will signify completion of the commissioning task.â€ (This reflects an â€˜Aâ€™ scope: itâ€™s detailed, itemized, and leaves little guesswork. Each deliverable is described with timing and criteria, so vendors know what to price and plan for.)

B (Good): The scope of work is well-defined overall, though maybe not with the exhaustive detail of an A. Major tasks and deliverables are listed, but a few minor specifics or acceptance details might be absent. There is little ambiguity about core requirements, but contractors might have to seek clarification on some secondary details. Example Excerpt: â€œContractor will resurface approximately 5 miles of roadway on Main St. Deliverables: a traffic management plan before work commences, weekly work progress emails, and a final project report. All work shall meet state DOT paving standards. (Note: Striping and signage replacement are included; curb ramp upgrades are excluded from this scope.)â€ (This â€˜Bâ€™ excerpt is quite clear on the main work and deliverables. It could be improved with more granular details â€“ e.g., specifying asphalt thickness or exact standards by number â€“ but itâ€™s sufficient for bidders to understand the job and deliverables.)

C (Average): The RFPâ€™s scope description is moderately clear but incomplete. It covers the general idea of the work, but important details (quantities, specific tasks, or boundaries) are missing. Deliverables might be mentioned in passing or not at all, or described in vague terms. Bidders can still formulate a proposal, but they may have to make assumptions or ask many questions. Example Excerpt: â€œScope: Renovate the interior of Building A as needed and provide improvements. Deliverables include project documentation and final results. The contractor will be responsible for most interior trades (painting, flooring, etc.).â€ (This â€˜Câ€™ scope is quite vague â€“ â€œas neededâ€ and â€œimprovementsâ€ are not defined, and deliverables like â€œproject documentationâ€ are not specific. Bidders would be unsure of the full extent â€“ e.g., how many rooms to paint? what constitutes an improvement? â€“ requiring assumptions or RFIs.)

D (Poor): The scope of work is highly vague, inconsistent, or missing key elements. It might list a project title or one-liner task without detail, omit major components, or use overly broad language (e.g., â€œContractor to do all necessary workâ€ with no further explanation). Deliverables are not clearly identified â€“ the RFP might not say what the outputs should be, or confuses deliverables with tasks. Bidders will likely be confused about what to include, increasing the risk of scope creep or disputes later. Example Excerpt: â€œScope: Contractor shall upgrade the facility to modern standards. All work must be high quality. Deliverables: N/A.â€ â€“ or â€“ â€œThe selected firm will provide services for the project as required.â€ (These â€˜Dâ€™ examples show almost no actionable detail. Contractors reading this have little idea what â€œupgradeâ€¦ to modern standardsâ€ specifically entails â€“ which systems, what level of upgrade? Such an RFP would prompt a barrage of questions or very cautious bids padded for the unknowns.)

F (Failing): The RFP provides practically no usable scope information. It might entirely lack a scope section, or the content is so fragmented/gibberish that itâ€™s impossible to tell what work is expected. In some failing cases, the RFPâ€™s scope description might even contradict itself (telling different stories in different sections about what the project includes). An â€˜Fâ€™ scope is a recipe for bidder confusion, wildly divergent proposals, or no bids at all. Example Excerpt: â€œScope of Work: TBD by the contractor. The contractor shall do everything necessary to complete the project. (No further detail provided.)â€ â€“ or â€“ â€œScope: Build the thing as per requirements (which will be given later).â€ (These examples are extreme, but illustrate an â€˜Fâ€™. The RFP basically abdicates defining the scope. Bidders are being asked to propose â€œeverything necessaryâ€ without guidance â€“ an untenable situation that would likely result in no compliant proposals or significant risk premiums.)
...
GRADING SCALE:
A (Excellent): The RFP clearly outlines all evaluation criteria and their importance (ideally with weights or an order of priority). It might present a table or list like: â€œProposals will be evaluated on: (1) Technical Approach â€“ 30%, (2) Team Qualifications â€“ 20%, (3) Past Project Experience â€“ 20%, (4) Project Schedule â€“ 10%, (5) Price â€“ 20%. The highest scoring proposal based on these factors will be selected.â€ Each criterion is well-described, so bidders know what the evaluators will be looking for under each (e.g., what constitutes a strong technical approach, what the client values in experience). The RFP also states the evaluation method (e.g., points scoring, committee review) and the award basis (best value vs lowest price, etc.). This leaves no guesswork â€“ vendors can align their proposals with what matters most and trust that the process is structured and fair. Example Excerpt: â€œEvaluation Criteria: A Selection Committee will evaluate proposals as follows: Project Understanding & Technical Approach (25%): We will assess the creativity, feasibility, and detail of your approach to the scope. Firm Experience & Key Personnel (20%): We will consider relevant project experience and staff qualifications (resumes provided). Schedule and Work Plan (15%): We will evaluate how realistic and well-thought-out your project schedule and phasing are. Cost Proposal (30%): The total proposed price will be scored based on a formula (lowest price gets full points). Quality of Proposal & Compliance (10%): Clarity and completeness of the proposal and adherence to RFP instructions. Basis of Award: The contract will be awarded to the proposal offering the best value overall, considering both technical merit and price.â€ (This â€˜Aâ€™ excerpt clearly delineates all factors with percentages and explanations. Bidders reading this know exactly what to emphasize. Itâ€™s transparent â€“ note even the price scoring method is mentioned â€“ and it aligns evaluation with the RFPâ€™s requests, which is ideal.)

B (Good): The RFP does list the evaluation criteria, but might not assign explicit weights or might group them broadly. Still, bidders can discern what the important factors are. For instance, it might say â€œWe will evaluate based on technical quality, experience, and cost, among other factors,â€ without numeric weights â€“ not as precise as an A, but at least the criteria are stated. Or perhaps weights are given but the description of each criterion is minimal. Overall, though, the playing field is mostly clear. Example Excerpt: â€œEvaluation Factors: The County will evaluate proposals on (a) Technical Solution, (b) Team Qualifications/Past Performance, (c) Schedule, and (d) Cost. These factors will be considered in determining the proposal offering the best value. Cost is important but will not be the sole determinant.â€ (This â€˜Bâ€™ example names the factors, indicating a best-value approach. It doesnâ€™t show exact weights or ranking, but bidders can tell these four areas all matter. Itâ€™s reasonably transparent, just not as detailed. Most vendors would be comfortable that they know the key areas to focus on, though they might prefer more insight into relative importance.)

C (Average): The RFP mentions some evaluation considerations but is vague or incomplete. It might have a boilerplate line like â€œAward will be made to the proposal deemed most advantageous to the Owner, price and other factors considered,â€ without enumerating what â€œother factorsâ€ are. Or it might list criteria but leave out one that later turns out to be used, or use imprecise terms (e.g. â€œqualityâ€ without definition). Bidders have a general sense that, say, both price and technical factors are in play, but not a clear picture of the weighting or all criteria. Example Excerpt: â€œBasis of Award: The City will select the contractor that it feels offers the best combination of capability, approach, and price. We will look at overall compliance with the RFP, the proposerâ€™s experience, and the proposed cost. The City reserves the right to make an award in its best interest.â€ (This â€˜Câ€™ statement is somewhat informative â€“ it implies experience, approach, and price matter â€“ but itâ€™s not a well-structured criterion list. â€œBest combinationâ€ and â€œbest interestâ€ are broad terms. Bidders can infer they should present a strong approach and competitive price, but they donâ€™t know if, for example, approach is more important than price or vice versa. Itâ€™s average transparency at best.)

D (Poor): The RFP provides little to no clarity on how proposals will be evaluated. It might be completely silent on criteria, or so general that itâ€™s unhelpful. In some cases, the RFP might hint at one thing but the actual evaluation might consider others not disclosed (e.g., an RFP that doesnâ€™t mention that interviews will be held or that a certain certification is a must, catching bidders off guard). Lack of transparency here can make vendors suspicious of a predetermined outcome or simply confused on how to craft their response. Example Excerpt: â€œProposals will be evaluated in accordance with internal policies. The contract will be awarded to the firm which, in the Cityâ€™s opinion, best meets the requirements. No specific criteria are being published.â€ Or: â€œEvaluation will be based on the overall value of the proposal to the Owner.â€ (These â€˜Dâ€™ examples are problematic â€“ they basically refuse to tell bidders how their work will be judged. Serious vendors might balk at this, as it suggests the decision could be arbitrary. It also prevents bidders from understanding what to highlight. This opaqueness often results in uneven proposals and erodes trust. It is far below best practice, where even a simple weighted list is expected.)

F (Failing): The RFP not only hides criteria, but possibly also shows signs of unfair or ad-hoc evaluation intentions. For example, an RFP that says â€œThe Owner will decide at its sole discretion based on secret factorsâ€ would be an egregious case. Or if the RFPâ€™s statements about evaluation are outright misleading or self-contradictory (e.g., in one place it says lowest price wins, elsewhere it implies a trade-off, leaving total confusion). An â€˜Fâ€™ might also be earned if the RFP introduces evaluation criteria after proposals (not in the RFP) or changes them without informing bidders â€“ but that would be seen after issuance. From the document alone, an â€˜Fâ€™ is when bidders have no idea how theyâ€™ll be judged, or have a wrong idea due to bad information in the RFP. Example Excerpt: â€œAward: The decision will be made by the Director based on any factors they deem relevant. Bidders will not be informed of the evaluation criteria or scoring methodology. The Owner reserves the right to award based on favoritism or any reason.â€ (While no RFP would state it so baldly, an implicit equivalent is when nothing is shared and the process is entirely opaque. Another failing example: an RFP that pretends to have criteria but they are so obviously unreasonable or not actually related to the requirements. For instance, listing a criterion like â€œProximity of firmâ€™s headquarters to Ownerâ€™s office â€“ 40%â€ for a project where location is irrelevant, potentially skewing competition unfairly. Such misaligned or undisclosed criteria violate the principle of true discriminators and fair competition, thus an RFP doing this would get an â€˜Fâ€™ for transparency and fairness in evaluation.)
...
GRADING SCALE:
A (Excellent): The RFP provides comprehensive and clear technical documentation. All relevant drawings (plans, elevations, etc.), specifications, and data are included or referenced. Technical requirements are explicitly stated (e.g., performance standards, materials to use, code requirements). There are no significant gaps: if site conditions matter, the RFP includes surveys or reports (e.g., soil report, existing utility maps); if design input is needed, preliminary drawings or reference designs are attached. Moreover, the RFP highlights key standards (e.g., â€œConstruction must comply with the 2021 International Building Code and OSHA 29 CFR 1926â€). Bidders have a complete picture of the technical context and requirements, minimizing assumptions. Example Excerpt: â€œTechnical Attachments: Provided in Appendix B are 30% design drawings (civil, structural, and architectural) for the new facility, a geotechnical investigation report with four soil borings, and an existing site utilities map. Specifications: The project shall adhere to ASTM and ACI standards as listed in Section 5 of this RFP â€“ for instance, concrete work must meet ACI 318-19 and reinforcing steel per ASTM A615 Grade 60. Where specific products are referenced, â€œor equalâ€ alternatives are permitted. Contractors must review the attached hazardous materials survey (Appendix C) to account for required abatement. Clarification: Any deviations from these requirements must be approved via addendum.â€ (This â€˜Aâ€™ excerpt illustrates an RFP that hands bidders a wealth of technical info â€“ drawings, soil data, standards â€“ ensuring they can bid accurately with minimal guesswork.)

B (Good): The RFP provides most key technical information, though a few minor details or documents might be missing or not as clear. For example, drawings and specs are provided but perhaps one ancillary report (say, an older utility plan) is not included, or certain technical requirements are implied rather than explicitly stated. Nonetheless, the missing pieces are not deal-breakers â€“ a reasonably experienced bidder can fill in the gaps with minimal uncertainty. Example Excerpt: â€œThe RFP includes schematic design drawings and a list of required building systems (HVAC, electrical, structural). All workmanship shall follow relevant codes (IBC, NEC, etc.). Bidders are expected to verify on-site conditions; a pre-bid site visit will be held. (Note: A full topographic survey will be provided to the awarded contractor.)â€ (This â€˜Bâ€™ example gives most information â€“ drawings at schematic level, code requirements â€“ but notes that a topo survey isnâ€™t provided now, which introduces some uncertainty. Itâ€™s still generally good; prudent bidders might raise questions or include assumptions for the missing survey, but the majority of technical expectations are communicated.)

C (Average): The RFP includes some technical information, but leaves notable gaps or ambiguities. Bidders have part of the picture but not all â€“ perhaps only a conceptual sketch or outline specs are given where detailed ones are needed, or critical site data is not provided. The RFP might rely on generic statements (â€œwork to be per standard practiceâ€) instead of project-specific requirements. Bidders will likely need to request information or make conservative assumptions, adding risk. Example Excerpt: â€œA basic floor plan sketch of the proposed layout is attached. Materials should be commercial grade and meet applicable standards. Specific design calculations or soil information are not available at this stage; contractors should account for typical soil conditions in this region. Any necessary drawings will be the contractorâ€™s responsibility to produce.â€ (In this â€˜Câ€™ scenario, the RFP gives only a rough idea (a sketch) and minimal technical guidance. There is no geotech data or detailed spec â€“ bidders would be forced to guess soil conditions and other design parameters, which could result in widely varying interpretations of scope and cost.)

D (Poor): The RFPâ€™s technical information is seriously lacking or confusing. Key drawings or specs are missing, or the provided information contains contradictions and errors. For instance, the written specification might call for one thing and an attached diagram shows another (e.g., a note says â€œuse steel grade Aâ€ but the drawing labels â€œsteel grade Bâ€) â€“ these kinds of conflicts cause bidder confusion. Little to no site data is shared even if the project depends on it. Bidders would have to make major assumptions or even invest in their own preliminary investigations just to bid, which many will avoid. Example Excerpt: â€œNo formal plans are available; bidders may visit the site and measure as needed. The RFP text mentions a 50 kW generator, but an older diagram (Fig. 1) labels it as 30 kW â€“ bidders should â€˜do their bestâ€™ to account for power needs. Material specs will be defined during construction. If information appears inconsistent, the contractor is responsible for reconciling it.â€ (This â€˜Dâ€™ excerpt illustrates a poorly prepared RFP: no official drawings, self-contradictory references (50 kW vs 30 kW), and punting specs to later. Such an RFP would raise red flags; responsible bidders might either not bid or pad their bid significantly to cover the unknowns.)

F (Failing): The RFP provides virtually no usable technical information, or is so inconsistent that itâ€™s unusable. For example, there are zero drawings or figures for a complex construction project, no specifications beyond â€œbuild to code,â€ and critical details (dimensions, capacities, site conditions) are omitted entirely. In worst cases, the RFP might explicitly state that the owner will not provide information (e.g., â€œno site data will be given, contractor assumes all conditionsâ€), dumping all risk on bidders. This is a recipe for project failure or no bids. Example Excerpt: â€œTechnical Requirements: Contractor to determine all requirements. No drawings are provided. Bidders should rely on their expertise to identify needed specifications. The facility must be built per all applicable laws â€“ details to be finalized after award. (The Owner provides no warranties on existing site conditions; unknown conditions are at contractorâ€™s risk.)â€ (This extreme â€˜Fâ€™ case basically says â€œyou figure it outâ€ to the contractor. It withholds all data and places full responsibility on bidders for technical scoping. Such an RFP is highly unprofessional â€“ it practically guarantees either no competent firm will bid or those that do will include huge contingency costs. It would likely lead to disputes or project collapse due to the unknowns involved.)
...
DEFINITION & IMPORTANCE: This variable evaluates whether the RFP is structured to promote fair, open competition and uses meaningful discriminators to select a winner. â€œFair competitionâ€ means the RFPâ€™s requirements and criteria are not unjustifiably biased or restrictive â€“ any qualified vendor should have a reasonable chance to compete. True discriminators are the factors that genuinely differentiate one proposal from another in terms of value; an RFP should focus on those, rather than arbitrary or overly specific demands that donâ€™t add value. An RFP that scores well here avoids favoritism (not written with a pre-selected vendor in mind), avoids needless barriers (like requiring a vendor have 50 years of experience when 5 would do), and emphasizes evaluation factors that correlate with project success (technical quality, cost, etc.) rather than trivialities. Ensuring fair competition also means giving all vendors equal information and not tailoring specs to one product without allowables (â€œbrand X or equalâ€ is okay; â€œmust use brand Xâ€ with no reason is not, unless only one solution truly exists). If the RFP is too restrictive without cause, competition suffers â€“ vendors might not bother or the process might be seen as a sham. Conversely, if itâ€™s too loose, that can be fair but might not help in selecting the best (so thereâ€™s a balance).
...
GRADING SCALE:
A (Excellent): The RFP is impartial and focuses on meaningful differentiators. The requirements are specific to the projectâ€™s needs but not tailored to any one vendor or product (unless justified and allowed with equivalents). For instance, instead of naming a proprietary system, it specifies performance criteria (â€œmust handle X throughputâ€) so multiple solutions can qualify. Any vendor meeting the qualifications has a fair shot â€“ there are no hidden preferences. The evaluation factors are all relevant and value-driven, helping distinguish the best proposal. Unnecessary criteria that wouldnâ€™t actually affect success are omitted (no â€œfluffâ€ or irrelevant hurdles). The RFP also explicitly encourages competition â€“ e.g., states that alternative approaches or innovation are welcome (when appropriate), and it refrains from overly punitive terms that would scare vendors off without benefit. In sum, itâ€™s written as if the issuer genuinely wants the best solution from the broad market. Example Excerpt: â€œThe City invites all qualified firms â€“ large or small â€“ to propose. No proprietary technology is mandated; proposals may utilize any solution that meets the performance requirements stated. The RFPâ€™s criteria (technical approach, past performance, and cost) are designed to select the proposal providing the best value, without bias toward any particular vendor or method. For example, while a certain software is used as a reference model, bidders may propose an equivalent or better system. The requirements focus on outcomes, not specific branded inputs, to ensure we get a range of competitive solutions. Additionally, the RFP limits requirements to those that add value: all evaluation factors are true discriminators that will help tell proposals apart â€“ there are no arbitrary minimums that donâ€™t impact project success.â€ (This â€˜Aâ€™ excerpt demonstrates a fair and open stance. It explicitly says no proprietary lock-in, encourages various solutions meeting performance needs, and claims all criteria are there for a reason. It implies the RFP was crafted to maximize competition while still homing in on what matters. It facilitates fair competition by design.)

B (Good): The RFP is generally fair and uses mostly relevant discriminators, but may have a few minor issues. Perhaps it has some stricter requirements that slightly narrow the pool, yet they are at least somewhat justifiable (for example, requiring a vendor to have done 3 similar projects â€“ that cuts out newbies, but is a reasonable qualifier for a big job). Or it might specify a preferred solution but allows equals (â€œBrand X or equivalentâ€), which is fine, though a completely neutral spec is ideal. The evaluation criteria mostly focus on important factors, but maybe one criterion is of debatable value (e.g., a small preference for local offices which, while giving local firms a boost, is not entirely project-performance related â€“ still, itâ€™s a common practice and usually disclosed). Importantly, a â€˜Bâ€™ RFP has no blatant favoritism or hidden agendas; any vendor reading it would feel they have a fair chance if they meet the requirements. Example Excerpt: â€œRequirements include that the contractor possess a valid state license and have completed at least two projects of similar size in the past five years. (This ensures experienced providers without unduly excluding qualified newer entrants who have some track record.) The RFP does mention a specific software platform the city currently uses for asset management and states a preference for integration, but it also notes that proposals offering alternatives will be considered if they meet integration requirements. Evaluation factors are largely substantive â€“ e.g., methodology, experience, cost â€“ with a 5% preference for local businesses as per city policy. All vendors receive the same information and there are no secret criteria.â€ (This â€˜Bâ€™ scenario is mostly fair. The experience requirement is not extreme, the software preference is flexible with justification, and a local preference is disclosed and relatively small. These might slightly shape the competition, but they donâ€™t amount to an unfair advantage for one known vendor. True discriminators like methodology and cost carry the majority weight. Vendors would likely accept these terms as generally fair, even if not perfectly level in every dimension.)

C (Average): The RFP is somewhat restrictive or includes criteria that arenâ€™t true differentiators, but not to an egregious extent. Maybe there are high minimum qualifications that limit competition more than necessary (e.g., requiring 10 years of operation when perhaps 5 would suffice), or very detailed specifications that lean towards a particular solution without outright excluding others. The RFP might also include a few odd requirements that donâ€™t clearly add value (for instance, a very specific format or a minor certificate not really relevant). However, these issues, while they might reduce the number of capable bidders or put some at a slight disadvantage, arenâ€™t so severe as to indicate intentional bias â€“ perhaps more a result of boilerplate or not thinking through necessity. The evaluation might have one or two factors that wonâ€™t actually distinguish much (like giving a few points for proposal formatting neatness â€“ which all serious bidders would likely do anyway), diluting focus on true discriminators. Example Excerpt: â€œThe RFP specifies that the contractor must have an office in-state and at least 20 full-time employees. It also asks for proof of ISO9001 certification (even though this project is straightforward construction where that standard might not directly impact results). While these criteria might eliminate some smaller firms, the city included them aiming for reliability. The technical specs detail a preferred brand of pipe fitting; equivalents are allowed but the approval process for an equivalent isnâ€™t clearly described, possibly discouraging alternates. Evaluation includes standard categories, but also a â€œProposal Completenessâ€ criterion worth 10% â€“ essentially checking if the proposal followed instructions, which serious bidders all would, so it may not truly set proposals apart. Thereâ€™s no overt single-vendor bias, but some requirements could be seen as overly restrictive or not crucial.â€ (This â€˜Câ€™ example shows moderate issues: in-state office and 20 staff might cut out newer or out-of-state players who could do the job, and ISO certification might not be relevant â€“ these could reduce competition a bit. The preferred pipe brand and unclear equivalency clause might nudge bidders to that brand to be safe, hinting at a possible bias or just laziness in spec writing. None of this absolutely rigs the bid (multiple firms likely meet these requirements), but itâ€™s not as open as it could be. The evaluation criterion for completeness is not a real discriminator since presumably all finalists will be complete â€“ itâ€™s somewhat a wasted factor. Overall, itâ€™s an average situation: not blatantly unfair, but not optimally competitive either.)

D (Poor): The RFP shows clear signs of unfairness or irrelevant criteria that likely skew or limit competition significantly. Perhaps the requirements are written so narrowly that only a very small handful of vendors (or one known incumbent) can meet them â€“ e.g., specifying dozens of must-have features or certifications that coincide with what one vendor offers. Or it mandates a proprietary technology with no allowance for â€œor equal,â€ effectively locking the solution to one source. There might also be overly harsh terms (like extremely high insurance levels or liability clauses) that smaller or new vendors canâ€™t accept, without justification to the project risk, thereby favoring big players. The evaluation criteria might include arbitrary factors that donâ€™t relate to performance (like giving heavy weight to a factor that most competitors are identical in, or something like â€œoffice must be within 5 miles of our officeâ€ giving a huge hometown advantage). These practices undermine true competition and suggest the RFP issuer either has a predetermined favorite or is not concerned with getting the best value through competition. Example Excerpt: â€œThe technical specification in the RFP calls for use of the AcmeCo Brand XYZ control system, no substitutions, and requires the contractor to have 15 years of experience using this proprietary system. Additionally, one evaluation criterion gives 15% weight to â€˜prior direct work with our agency,â€™ which heavily favors the incumbent contractor. Together, these conditions practically ensure only one or two vendors qualify. The solicitation also demands a net worth of $100 million and worldwide project experience â€“ which is unrelated to this local projectâ€™s scope but excludes many capable mid-sized firms. These requirements are far more restrictive than necessary and limit competition.â€ (This â€˜Dâ€™ example is quite pointed: a brand lock-in with no substitution, a lengthy specific experience requirement (15 years) that aligns with a known vendor, and even an evaluation bias for having worked with the agency before â€“ all are hallmarks of an unfair RFP. It reads as if itâ€™s designed for one vendorâ€™s qualifications. Most vendors would see this and suspect itâ€™s a â€œwiredâ€ solicitation. Even if not intentionally corrupt, itâ€™s poor practice because it doesnâ€™t focus on real value differences â€“ e.g., insisting on one brand rather than performance, or 15 years vs evaluating actual capability. The result is likely fewer bids and possibly higher prices due to reduced competition.)

F (Failing): The RFP is effectively non-competitive or sham â€“ it either explicitly or implicitly excludes all but a preselected vendor, or uses nonsensical/irrelevant criteria that have nothing to do with the project just to tilt the playing field. An example is an RFP that requires a combination of credentials or product features so unique that only one company in the world could satisfy it (and those requirements arenâ€™t truly justified by project needs). Or an RFP that might even name a vendor or product without alternatives in a non-standard way, or include secret criteria (not disclosed) that are used to eliminate others. Another failing scenario: criteria that are supposed to matter but donâ€™t actually differentiate any bidder, combined with a lack of real factors, making the selection arbitrary â€“ but typically, failing fairness is about bias and excessive restriction. Example Excerpt: â€œThis RFP requires the contractor to have completed exactly four projects in the past year, each using the MegaBuild 3000 system, and at least one project for Department X (the issuing department) â€“ conditions that only the incumbent contractor meets. It also states that the decision has already been preliminarily made based on market research, and proposals are a formality. The evaluation criteria list includes odd items like the astrological sign of the companyâ€™s CEO (obviously not really relevant) â€“ indicating a lack of genuine intent to evaluate fairly. Essentially, no other firm can realistically compete under these terms.â€ (This exaggerated â€˜Fâ€™ scenario highlights an RFP that is ostensibly a farce â€“ it either intentionally or negligently ensures only one outcome. The requirement of â€œexactly four projectsâ€ with a specific system and having worked for the department is absurdly narrow and tailor-made. The mention that a decision is already made would never be put so bluntly, but sometimes insiders or wording might tip off that they already have someone in mind. When criteria become irrelevant or comical (like the astrological sign example for effect), it signals the evaluation is not serious. In short, an â€˜Fâ€™ RFP violates the principle of fair, competitive procurement. Regulators or ethical oversight would score it extremely poorly, and vendors would likely either protest or not bother, knowing itâ€™s a done deal.)
...
DEFINITION & IMPORTANCE: This measures how clearly the RFP explains why the project is being undertaken, what success looks like, and any background context (historical, technical, or organizational) that frames the project. A high-quality RFP provides a concise project purpose statement, specific objectives/outcomes, and relevant context such as current challenges or drivers for the project. This context is crucial because it aligns bidders with the projectâ€™s goals and helps them tailor their proposals effectively. In fact, a well-crafted context section can â€œmake or breakâ€ an RFP â€“ it gives vendors a clear rationale and motivation, leading to more relevant and targeted proposals. Conversely, a poor RFP in this area leaves bidders guessing the projectâ€™s true goals or background, risking misaligned proposals.
...
GRADING SCALE:
A (Excellent): The RFP articulates a crystal-clear purpose and desired outcomes, and provides rich project context. It explains the problem or need driving the project, the intended results (with measurable targets if applicable), and background info such as history, stakeholders, or strategic importance. The narrative is cohesive and motivating, giving vendors a full picture of â€œwhyâ€ the project exists. Example Excerpt: â€œBackground: The Cityâ€™s 2030 Vision identifies aging water infrastructure as a critical challenge. This RFPâ€™s purpose is to modernize Pump Station #5 to increase capacity by 25% and meet new EPA standards. Desired Outcome: A fully operational, energy-efficient pump station that eliminates current overflow incidents (baseline: 4 per year). By sharing our recent system assessment (Appendix A) and regulatory context, we ensure proposers understand the urgency and performance goals of this project.â€ (This excerpt clearly states why the project is needed, the specific outcome targets, and context of assessments/regulations, reflecting an â€˜Aâ€™ quality RFP.)

B (Good): The RFP states the projectâ€™s purpose and goals, but with slightly less detail or clarity. The context is provided but may omit some nuances. Overall understanding is still good â€“ bidders can tell what is needed and why â€“ but perhaps the outcomes arenâ€™t quantified or the background could be richer. Example Excerpt: â€œThe purpose of this project is to upgrade the campus security system to address recent incidents and comply with updated safety policies. The expected outcome is a more reliable, modern system across all facilities. (Context: our current system is 15 years old and prone to failures.)â€ (This is generally clear, though it might not include specific metrics or a deep dive into incident stats; itâ€™s a solid â€˜Bâ€™ â€“ informative but not as comprehensive as it could be.)

C (Average): The RFP provides some statement of purpose and outcomes, but itâ€™s vague or incomplete. The context is minimal â€“ maybe just a sentence or two of background with little detail. Bidders can infer the basics of whatâ€™s needed, but they might not fully grasp the projectâ€™s significance or specific success criteria. Example Excerpt: â€œThe City is seeking proposals for construction of a new community center. The goal is to improve services for residents. (Background: The current facility is outdated.)â€ (This â€˜Câ€™ example gives a general idea â€“ build a community center to improve services â€“ but lacks detail on what â€œimprove servicesâ€ means, how outdated the current facility is, or any specific objectives. Itâ€™s serviceable but not specific.)

D (Poor): The RFPâ€™s purpose and outcomes are unclear or conflicting. It may merely repeat a title (â€œConstruction of X projectâ€) without explaining why itâ€™s needed or what the owner expects to achieve. Little to no project context is given, or the information provided raises questions (e.g. mentions a problem but provides no data, or lists objectives that donâ€™t align). Bidders will be confused about the driving vision. Example Excerpt: â€œThe Department requests proposals for Project Alpha. (No further explanation of project rationale.) Expected outcome: completion of Project Alpha per specifications.â€ (This â€˜Dâ€™ excerpt is essentially just naming the project; it doesnâ€™t state any real purpose or context â€“ bidders donâ€™t know why the project matters or what success entails beyond generic completion.)

F (Failing): The RFP fails to communicate the projectâ€™s purpose or outcomes. It may jump straight into technical specs or tasks with zero contextualization, or provide a few disjointed statements that confuse more than clarify. An â€˜Fâ€™ RFP leaves bidders in the dark about the â€œbig picture,â€ often resulting in misaligned proposals or lack of bidder interest. Example Excerpt: â€œThe Company is issuing this RFP for unspecified services. Outcome: TBD. Context: N/A.â€ â€“ or â€“ â€œWe seek a solution; details will be discussed with the winner.â€ (In these extreme examples, the RFP gives virtually no insight into why the project exists or what it aims to achieve. Such an RFP is unacceptable, as vendors cannot divine the projectâ€™s intent, leading to wildly varied or misguided proposals.)
...
GRADING SCALE:
A (Excellent): The RFP sets forth a clear, feasible schedule for the project (and the procurement process) that aligns with the projectâ€™s complexity. It lists key milestones and dates in a logical sequence and provides rationale for any aggressive dates. For example, it might include a project timeline Gantt or table: design phase duration, permitting period, construction start and substantial completion dates, etc., all of which appear reasonable. If the project has a firm deadline (like a grant expiration or event date), the RFP explicitly mentions it and possibly invites bidders to propose alternatives or confirm feasibility. The RFP also clearly states the proposal timeline (when bids are due, when award is expected, etc.) with enough time for bidders to prepare quality proposals. Example Excerpt: â€œProject Schedule: The anticipated contract award is March 1, 2026. Construction is expected to start by April 2026 after permits are obtained, with a substantial completion milestone of March 31, 2027 (12 months on-site construction). Major milestones: 30% design review by July 2026; 60% design by Oct 2026; Begin foundation work by Nov 2026; Commissioning and testing in Feb 2027. These dates are based on a required operational date of May 2027 to align with the new school year. Note: Bidders should confirm in their proposals that this timeline is achievable; slight adjustments can be negotiated if justified. The RFP includes a detailed schedule table and identifies dependencies (e.g., â€œOwner-furnished equipment delivery by Dec 2026â€).â€ (This â€˜Aâ€™ example shows a detailed, sensible timeline. It spans a year+ for a significant project, which is plausible. The deadlines are justified by an operational need and the RFP is transparent about them, even allowing discussion of slight adjustments â€“ a very mature approach.)

B (Good): The RFP provides a schedule or completion deadline that is generally reasonable, though perhaps on the tighter side or lacking some detail in milestones. It lists the final completion date and maybe a couple of interim dates, and these seem achievable, albeit with efficient work. There might be minor concerns (e.g., not much float time for unexpected delays), but overall the timeline isnâ€™t wildly unrealistic. The bidding timeframe is also adequate (not last-minute). Example Excerpt: â€œProject completion is required within 8 months of contract award. The desired milestones are: foundation complete by Month 3, structural framing by Month 5, and final inspections in Month 8. We plan to award by September 1, with work commencing immediately. (The timeline is driven by the funding cycle which ends next fiscal year.) Bidders should include a schedule confirming they can meet these milestones.â€ (This â€˜Bâ€™ schedule is relatively tight â€“ 8 months â€“ but not impossible for, say, a moderate-sized building. It does provide interim milestones. It could be improved by adding detail or buffer, but itâ€™s still mostly credible, especially since a reason is given and bidders are asked to confirm feasibility.)

C (Average): The RFP mentions a schedule or end date, but itâ€™s either somewhat ambiguous or questionable in realism. It might just specify a completion date without considering whether the scope can realistically be done in that time, or it may omit intermediate milestones which would help understand the pace. Alternatively, the schedule could be realistic but not clearly communicated (scattered references to dates in different sections, for example). The bidding timeline might also be just barely adequate, indicating moderate quality. Example Excerpt: â€œThe project should be completed as soon as possible, ideally by Q4 of this year. A detailed timeline is not provided; bidders should propose a schedule. (Owner anticipates construction might take ~6 months.)â€ (In this â€˜Câ€™ case, the RFP suggests a desired completion by end of year but doesnâ€™t firmly commit or detail milestones. â€œAs soon as possibleâ€ is open-ended and the owner guess of 6 months might or might not be accurate. Bidders are left to figure out the schedule themselves. Itâ€™s not outright crazy, but itâ€™s not well-defined either, putting it in the average range.)

D (Poor): The RFPâ€™s schedule expectations are unrealistic or very unclear. Perhaps the RFP demands a project timeline that is overly aggressive given the scope (e.g., expecting a normally 12-month project to be done in 4 months). It might also cram the bidding and award process into an unreasonably short window. Another scenario: the RFP gives contradictory schedule info (one section says completion in 6 months, another elsewhere says 12 months). Such issues can dissuade qualified bidders, who recognize the schedule is not feasible. Example Excerpt: â€œSchedule: Time is of the essence. The facility must be fully constructed and operational 90 days from contract award, no exceptions. (Note: A different page of the RFP mentions a 6-month schedule; if conflicting, assume the shortest timeframe.) Also, the RFP was issued on June 1 with proposals due June 7 â€“ we expect to break ground by June 15.â€ (This â€˜Dâ€™ scenario is clearly problematic: a 90-day build for something presumably big, plus a one-week bid period, indicate a rushed/unrealistic plan. The internal inconsistency (90 days vs 6 months) adds to the confusion. Most experienced bidders would view this as high-risk or impossible without massive resources or corners cut.)

F (Failing): The RFP either provides no schedule guidance at all for a time-sensitive project, or sets a completely impossible timeline that betrays a lack of understanding. It might ignore obvious constraints (e.g., expecting outdoor construction in deep winter in a cold region without plan) or provide a timeline that no rational contractor could meet (leading to either project failure or no bids). Another failing case is when an RFP timeline is so chaotic â€“ for example, requiring proposal submission, evaluation, award, and full project completion all in an absurdly short span â€“ that it suggests the RFP issuer is not serious or competent. Example Excerpt: â€œSchedule: ASAP. We require all work done and delivered within one month of award, regardless of project size. The specific dates will be determined later. Bidders should be prepared to work 24/7 if needed. No timeline extensions will be granted.â€ Or, an example from a process standpoint: â€œRFP issued Nov 1. Questions due Nov 7 (only 6 days for questions). Answers by Nov 8. Proposals due Nov 9 (only 1 day after Q&A). Interviews on Nov 12, award by Nov 14, and project kickoff Nov 15. Final deliverable must be live by Dec 15.â€ (These are drawn from a real example of a highly unrealistic schedule. This kind of timeline is highly unlikely to succeed: bidders have virtually no time to prepare after Q&A, the owner cannot realistically evaluate and approve in the scant days allotted, and the project timeline is extremely compressed. An RFP of this nature would earn an â€˜Fâ€™ â€“ it demonstrates poor planning and sets the project up for failure or no participation from credible vendors.)
...
GRADING SCALE:
A (Excellent): The RFP is fully internally consistent. Every major requirement in the scope is reflected in the proposal instructions and evaluation scheme. There are no conflicting statements or numbers across sections. For instance, if the scope says â€œinclude commissioning services,â€ the proposal instructions explicitly ask for a commissioning approach, and the evaluation criteria include assessment of the commissioning plan. The schedule for proposal submission and evaluation described in one section matches any dates mentioned elsewhere. The RFP might even provide a compliance matrix or cross-reference chart to map requirements to proposal sections and evaluation factors, underscoring the tight alignment. Example Excerpt: â€œSection III (Scope of Work) details a requirement for a Quality Control Plan to be implemented during construction. In Section IV (Proposal Instructions), we ask bidders to â€œdescribe your proposed Quality Control Planâ€. In Section V (Evaluation Criteria), the Quality Control Plan is explicitly one of the sub-factors under Technical Approach. (Likewise, every item the proposal must include corresponds to something we will evaluate, and nothing is evaluated that we didnâ€™t ask for.)â€ Another indicator in an excerpt: â€œThe RFP includes a Requirements-to-Proposal Crosswalk Table (Appendix E) that lists each RFP requirement and indicates where in the proposal it should be addressed and under which evaluation factor. Offerors are encouraged to use this as a checklist to ensure full compliance.â€ (This kind of meticulous consistency exemplifies an â€˜Aâ€™ â€“ the RFP was clearly drafted with coordination, making it easy for bidders to follow and cover all points. There are no discrepancies like differing numbers, terms, or instructions anywhere in the document.)

B (Good): The RFP is mostly consistent, with only minor lapses or omissions in alignment. Perhaps 95% of the requirements line up with instructions and criteria, but one small element might be missing â€“ for example, the RFP requires a certain plan (like a sustainability plan) in the work, but it forgets to explicitly ask for it in the proposal outline, though an attentive bidder could infer to include it. Or maybe the evaluation criteria combine some factors in a way not exactly mirrored by how the instructions were written (e.g., the instructions separate â€œStaffing Planâ€ and â€œExperienceâ€ sections, but evaluation combines them into one factor â€“ not a deal-breaker, but a slight inconsistency). There are no blatant contradictions, just subtle misalignments or lack of one-to-one correspondence. Example Excerpt: â€œThe RFP asks in Section 2 for a list of key sub-consultants. While the evaluation criteria donâ€™t explicitly name â€˜sub-consultantsâ€™ as a scored item, they are effectively considered under the â€˜Team Experienceâ€™ factor. All other requested proposal items (approach, schedule, etc.) have matching evaluation factors.â€ (This illustrates a minor inconsistency: sub-consultants info is requested but not separately scored â€“ likely folded into a broader category. A bidder might wonder if itâ€™s truly considered, but itâ€™s a small issue. Overall, the RFP would still come across as well-aligned; only a very detail-oriented bidder might notice a slight gap.)

C (Average): The RFP has some noticeable inconsistencies or gaps, though not catastrophic ones. Perhaps certain requirements arenâ€™t mentioned in the evaluation or instructions, or vice versa. For example, the RFP might require compliance with a specific standard or a certain service (like â€œmaintenance for 2 years after installationâ€) in the scope, but then never ask the bidder to address that in their proposal, nor indicate it will be evaluated â€“ leaving a grey area. Or maybe the instructions ask for a detailed staffing plan, but the evaluation criteria donâ€™t clearly state that staffing will be evaluated (it might be lumped into â€œoverall approachâ€ without saying so). Additionally, there could be minor conflicting info â€“ like the RFP document says proposals must be 30 pages in one section and 40 pages in another (confusing, but not directly about content). These issues can cause bidders to guess whatâ€™s important or possibly overlook something that is required but not emphasized consistently. Example Excerpt: â€œSection III lists a â€˜Community Outreach Planâ€™ as a project requirement, but when preparing the proposal, bidders find no mention of an Outreach Plan in the proposal instructions. The evaluation criteria also donâ€™t explicitly refer to outreach or communication. (Itâ€™s unclear if or how the outreach aspect will be considered in selection.) Additionally, the RFPâ€™s Section I says the proposal should include a timeline, but Section IV (Instructions) doesnâ€™t list â€˜Timelineâ€™ as a required section â€“ this inconsistency could confuse bidders whether to include a separate schedule narrative.â€ (This â€˜Câ€™ level scenario shows a couple of alignment issues. Bidders might notice the outreach requirement and decide to include it proactively, but some might miss it since it isnâ€™t asked for in the proposal instructions â€“ leading to uneven responses. Itâ€™s not an utter mess, but itâ€™s sloppy enough to reduce clarity.)

D (Poor): The RFP displays significant internal inconsistencies or misalignments. Multiple instances arise where one part of the RFP contradicts another or important elements donâ€™t sync. For example, the scope might require ABC, but the proposal instructions ask for XYZ instead, or evaluation criteria include factors that were never requested in proposals (surprising bidders). Numbers or terms might conflict (e.g., different sections of the RFP give different contract durations or different required completion dates). It may appear as if different people wrote different sections and they were never reconciled. This can lead to confusion, vendor questions, or even legal disputes if a bidder feels misled. Example Excerpt: â€œIn the Scope of Work, the RFP states the contractor must have capability to perform environmental testing; however, the Evaluation Criteria make no mention of environmental capability and instead include a scoring category for â€˜marketing planâ€™ â€“ which is not asked for anywhere in the RFP. The Instructions to Bidders request a â€œdetailed safety planâ€ in the proposal, but the Scope never mentioned safety planning as part of the project deliverables. Furthermore, Section I says the contract term is 2 years, while an attachment says 1 year â€“ an internal contradiction.â€ (This â€˜Dâ€™ example highlights multiple inconsistencies: an irrelevant evaluation factor (marketing plan) not grounded in any requirement, a proposal instruction (safety plan) that isnâ€™t reflected in the scope or evaluation explicitly, and conflicting basic info (contract term). These issues likely result in bidder confusion, necessary clarifications via Q&A, and a sense that the RFP is poorly cobbled together. It undermines confidence that the evaluation will be fair or focused on the right things.)

F (Failing): The RFP is riddled with contradictions and misalignments to the point of incoherence. Requirements, instructions, and evaluation seem to be on different planets. For instance, the RFP might include a copy-pasted instruction section from a completely different project, leading to irrelevant proposal requests and mismatched evaluation plans. You might see outright conflicts like the RFP telling bidders to ignore whatâ€™s said in another section (e.g., â€œDisregard Section 5 if it conflicts with Section 3â€ â€“ indicating a failure to reconcile). An â€˜Fâ€™ RFP might, for example, evaluate based on criteria that have no relation to the stated project objectives, or require proposal elements that have nothing to do with what will actually be delivered. This level of inconsistency can lead to mass bidder confusion, protests, or no viable responses. Example Excerpt: â€œ(Scope Section) The project is a bridge construction; (Evaluation Section) Proposals will be scored on the quality of graphic design and layout of marketing materials. â€“ (Instructions Section) Please include a proposed curriculum for student training programs (note: there is no mention of training in scope or evaluation). Also, Section 2 says â€˜Offerors must propose using software Xâ€™, while Section 6 says â€˜Any software is fineâ€™. The document repeatedly contradicts itself.â€ (This exaggerated but illustrative â€˜Fâ€™ scenario shows a completely disjointed RFP â€“ evaluation criteria about marketing for a bridge project, asking for a training curriculum thatâ€™s irrelevant, and direct self-contradictions. Itâ€™s almost nonsensical. Bidders would likely either be utterly confused or assume the RFP is a cut-and-paste error document. This kind of internal inconsistency could lead to 1 in 3 bids being disqualified due to â€œhidden or conflicting requirementsâ€ and, if contested, could result in legal challenges. In summary, an RFP at this level fails to provide a coherent basis on which to prepare or evaluate proposals.)
...
GRADING SCALE:
A (Excellent): The RFP provides a clear, fair draft contract or term sheet for the project, indicating major commercial provisions up front. Important terms like payment schedule (e.g., monthly progress payments, retainage %), warranty period, liquidated damages (if any), bonding requirements, insurance coverage limits, indemnification clauses, etc., are either included or referenced. The risk allocation is relatively balanced and industry-standard â€“ the owner retains certain risks that are better controlled by them (e.g., regulatory changes, differing site conditions might be handled via change order) and the contractor is assigned the risks they can control (e.g., workmanship, meeting the specs). If itâ€™s a public sector RFP with non-negotiable terms, the RFP plainly states that and provides the terms so bidders can accept or not; if negotiations are allowed, the RFP outlines that process (e.g., â€œAfter selection, the owner is willing to negotiate reasonable changes to terms except those mandated by lawâ€). The key is that bidders know the rules of engagement and the deal theyâ€™re bidding on. There are no hidden â€œgotchaâ€ clauses later â€“ transparency is high. Also, the terms arenâ€™t so draconian that only desperate or uninformed contractors would accept; rather, they resemble standard contracts (like ConsensusDocs, AIA, or other fair contract models) or clearly explain any deviations. Example Excerpt: â€œA Draft Contract is attached (Attachment 1) which will form the basis of the agreement. It includes: Payment Terms â€“ monthly progress payments, 10% retainage released at completion; Warranty â€“ Contractor to warrant work for 1 year after final acceptance; Liquidated Damages â€“ $500/day for late completion beyond a 30-day grace period, due to contractor delays only; Insurance â€“ minimum $2M general liability, etc.; Bonding â€“ 100% Performance and Payment Bonds required. Risk Allocation: The contract uses a differing site conditions clause â€“ if unknown site issues arise, the parties will equitably adjust price/time (i.e., contractor not solely at risk for hidden conditions beyond what a diligent investigation would show). Negotiation: The Cityâ€™s general terms are non-negotiable, but the City may discuss the project-specific scope and schedule with the selected proposer before finalizing the contract. Bidders should review all terms now and raise any exceptions with their proposal. The intent is a fair agreement â€“ for instance, the contract includes a mutual indemnification (each party holds the other harmless for its own negligence) rather than a one-sided indemnity.â€ (This comprehensive â€˜Aâ€™ example shows clarity and balance. It enumerates key points, shows how risk like unknown site conditions are handled fairly (with equitable adjustment, not just contractorâ€™s problem), and includes reasonable LDs and warranty. It also makes negotiation rules clear â€“ essentially, â€œthese are the terms, minor tweaks might be discussed but donâ€™t expect overhaul.â€ Bidders seeing this can price accordingly, not needing a huge risk contingency. The fairness â€“ e.g., mutual indemnity â€“ will encourage serious firms to bid because the contract isnâ€™t out to victimize them.)

B (Good): The RFP does include commercial terms, but they might be somewhat more owner-favorable or less detailed than an â€œAâ€ case, yet still within a normal range. For example, it might include a standard contract template that slightly tilts risk to the contractor, but nothing outrageous â€“ experienced bidders will recognize the terms and know how to manage them (perhaps by a bit of added contingency or insurance). Key terms are disclosed, but maybe a few specifics are missing (perhaps the exact insurance limits or indemnity clause text isnâ€™t provided in the RFP, but itâ€™s referenced that standard clauses will apply). Risk allocation might be a bit conservative â€“ e.g., a clause that says â€œcontractor is assumed to have examined site conditions thoroughly and no extra will be paid for the unknownâ€ â€“ which shifts risk to contractor, but this is at least stated. Bidders can factor it in. Negotiation might not be mentioned, implying either itâ€™s not expected or will follow standard practice; this lack of clarity is a small ding, but bidders usually assume limited negotiation if not stated. Overall, the terms are somewhat one-sided but not egregiously so, or theyâ€™re fair but not thoroughly explained (leaving bidders to assume standard practice). Example Excerpt: â€œThe RFP includes summary commercial terms: Payment will be lump-sum with 5% retainage; a one-year warranty on all work is required; a Performance Bond is required. The contract will likely be the Agencyâ€™s standard construction contract (available upon request) which contains typical provisions such as indemnification (Contractor indemnifies Agency for all claims arising from the work) and a no-damages-for-delay clause (time extension is sole remedy for owner-caused delays). While these terms place more performance risk on the contractor, they are standard for our projects. Bidders should be prepared to accept the contract as-is; extensive negotiations are not anticipated.â€ (This â€˜Bâ€™ example shows a bit more risk pushed to contractor â€“ e.g., no-damages-for-delay means if the owner delays, contractor only gets time, not cost, which is somewhat owner-friendly. Also the indemnity is one-directional. These are common but not ideal for contractors. However, theyâ€™re transparent here. Bidders will recognize them and either decide itâ€™s still worth bidding (maybe adding a contingency for delay risk) or not. The terms arenâ€™t hidden â€“ they said itâ€™s the standard contract and even offered it on request, which is reasonably transparent. Itâ€™s not balanced enough for an A, but itâ€™s acceptable and common, thus a â€˜Goodâ€™ rating.)

C (Average): The RFP addresses commercial terms and risk allocation to a degree, but there may be important details missing or somewhat onerous terms without full context. Possibly the RFP just says â€œstandard contract terms will applyâ€ but doesnâ€™t provide them up front â€“ leaving bidders to either assume or find those separately. Or it includes a few major terms but omits others (maybe it mentions a bond and warranty, but doesnâ€™t mention how changes or delays are handled). Risk allocation might default to the contractor by silence (e.g., it doesnâ€™t talk about differing conditions, implying the contractor might bear that risk by default). The RFP might also state that â€œthe contract is attachedâ€ but itâ€™s full of legalese that strongly favors the owner, which bidders have to comb through â€“ itâ€™s disclosed, but not highlighted or summarized, so some bidders might miss tricky clauses. Negotiation rules are likely not specified â€“ one might not know if exceptions to terms can be proposed or not, which is a grey area. Overall, an average case: terms not fully transparent or balanced, but also not shockingly bad; bidders might have to ask questions or assume worst-case on unknowns. Example Excerpt: â€œKey contract conditions include a requirement for 10% bid bond and 100% performance bond, and the contractor must carry $5M in liability insurance. The RFP does not explicitly state other terms, but bidders are expected to review the Draft Agreement (which was mentioned as available on the website, though not directly attached here). The draft contract contains standard government contract clauses. Notably, it has a broad indemnification clause and does not mention differing site conditions (implying the contractor assumes that risk). There is also a clause allowing the owner to terminate for convenience. These terms are common in our jurisdiction, but the RFP document itself did not highlight them. Bidders can seek clarifications if needed. Changes to contract language are generally not entertained after selection, although the RFP does not spell out a formal negotiation policy.â€ (In this â€˜Câ€™ scenario, the terms are out there but not clearly communicated in the RFP text â€“ bidders have to go look at the draft contract elsewhere to find important risk allocations like no differing site conditions clause. Some bidders might overlook that until late. The terms themselves (broad indemnity, termination for convenience) are standard in government work but all favor the owner. Still, nothing is extremely unusual â€“ just not balanced or clearly summarized. This requires bidders to be savvy and cautious, making it average. Communication of negotiation is vague â€“ likely meaning assume no negotiation but it wasnâ€™t plainly stated. This lack of clarity brings it down to average.)

D (Poor): The RFPâ€™s commercial terms are heavily one-sided, unclear, or likely to deter competition due to risk. Perhaps the draft contract (if provided) is very punitive: e.g., unlimited liability for the contractor, â€œno damages for any delay of any kindâ€ even if owner fault, requirement for extremely high insurance or bonding beyond normal (driving up cost or excluding smaller firms), etc. And the RFP either doesnâ€™t acknowledge how onerous these are or leaves them buried. Risk allocation might be so skewed that sophisticated bidders either no-bid or significantly increase their price to compensate. For instance, the contractor might be made responsible for all design flaws (even if the design was by the owner) or for any unforeseen event (force majeure included). Negotiation might be explicitly disallowed (â€œtake it or leave itâ€) combined with these harsh terms, leaving no room to discuss them. The RFP might also present some terms inconsistently â€“ maybe an RFP body text says one thing but the attached contract says another (leading to risk of later conflict). A poor rating also if the RFP simply omits mention of terms entirely (so you donâ€™t know what youâ€™re signing up for until after selection â€“ a major risk for bidders). Example Excerpt: â€œThe attached contract makes the contractor assume all risks: for example, there is a clause that any errors in the provided specifications are the contractorâ€™s responsibility to fix without extra cost (even though the design is owner-furnished). There is a strict â€œno claim for delayâ€ provision, even for owner-caused delays or unforeseen events â€“ meaning the contractor carries schedule and cost risk for everything, including acts of God. Liquidated damages are set at a high amount with no reciprocal early completion bonus. The payment terms include a very slow pay schedule (final 20% only upon project acceptance, many months after work). The RFP states these terms are non-negotiable. Many of these provisions are more onerous than industry norm, effectively shifting most project risk entirely to the contractor. The RFP document did not summarize these; only by reading the fine print of the draft contract does a bidder discover them. This likely results in higher bid prices or fewer bidders, as only those willing to shoulder significant risk will proceed.â€ (This â€˜Dâ€™ case describes a contract where virtually every risk (design errors, delays, etc.) is dumped on the contractor and commercial terms (like payment timing) are unfavorable. Such one-sided contracts often lead to inflated bids or scare off experienced firms, leaving either risky low bidders or no bidders. Since the RFP doesnâ€™t openly address them, itâ€™s even worse because bidders might get blindsided. This is poor practice â€“ while some strong owners try these terms, itâ€™s widely advised that severely unbalanced risk allocation reduces project success chances. The lack of transparency and flexibility pushes it to a D.)

F (Failing): The RFP either completely fails to inform about commercial terms (so bidders are essentially bidding into a black box contract), or the terms that are indicated are so extreme that no rational contractor would willingly agree except at exorbitant cost or under duress. For instance, an RFP that does not provide any contract or terms but requires a firm fixed price â€“ contractors have to assume the worst or gamble on unseen terms (a recipe for disaster or disputes). Or an RFP might include something outrageous like requiring the contractor to take on unlimited consequential damages, or to indemnify the owner for the ownerâ€™s own negligence, or to finance the project entirely and only be paid years after completion. If negotiation rules are also absent or forbid changes, itâ€™s essentially â€œsign your life away blindly,â€ which is a fail. Another failing scenario: contradictory or legally unenforceable terms that show the issuerâ€™s incompetence (e.g., clauses that conflict or violate law, making the commitment unclear). Example Excerpt: â€œNo draft contract or general conditions are provided with this RFP. Bidders must submit a price and will later be required to sign whatever contract the agency presents, with no modifications allowed. The RFP does, however, specify that the contractor will be responsible for all conceivable risks and losses â€“ including those arising from ownerâ€™s actions or third-party issues â€“ and that any cost overruns of any amount will not be paid, even if due to changed conditions or scope growth. Furthermore, the RFP demands that the contractor waive all rights to claim and that any dispute will be unilaterally decided by the ownerâ€™s project manager. Essentially, the contractor has no recourse once contracted. Payment terms or schedule are not defined at all in the RFP. These conditions create an impossible-to-assess risk. This approach is tantamount to asking contractors to sign a blank check of liability.â€ (This â€˜Fâ€™ scenario is extreme but captures the essence: bidders are given no contract to evaluate, told they canâ€™t negotiate, and the bits that are mentioned sound incredibly unfair (no claims, all risks, unilateral decisions). No sensible contractor would be comfortable â€“ theyâ€™d either walk away or put in a sky-high bid. The RFP fails to achieve a fair allocation or even a clear contract promise. This is a textbook case of how not to do it, as one-sided provisions often lead to fewer bidders and higher costs or disputes later. In summary, failing in this category means the RFPâ€™s approach to commercial terms and risk is so poor that it undermines the entire procurementâ€™s credibility or viability.)
...
GRADING SCALE:
A (Excellent): The RFP explicitly encourages innovation and outlines where trade-offs are possible. It defines the performance outcomes or objectives clearly and then states that bidders are welcome to propose creative or alternative methods to achieve those outcomes, rather than dictating a single solution. It might use language like â€œBidders may propose alternative technical solutions or methodologies if they can demonstrate equal or superior results. Innovative approaches that improve efficiency or reduce cost are encouraged.â€ It also gives guidance on trade-offs: for example, if budget is fixed, it might ask bidders to suggest scope adjustments or add-ons as options; or it might clarify priorities (e.g., â€œWhile cost is important, the City is willing to consider a higher initial cost for significantly reduced long-term maintenanceâ€). Essentially, it helps bidders understand how to optimize their design/plan for best value. Additionally, an RFP at this level often avoids overly prescriptive specs that box in the solution â€“ it might specify required outcomes (capacity, durability) but not micromanage the means, thereby preserving flexibility for vendor expertise. Example Excerpt: â€œInnovation and Alternatives: This RFP focuses on performance outcomes (e.g., the bridge must support X load and have a service life of 75 years). We have provided reference drawings, but bidders are encouraged to propose alternative designs or construction techniques that meet or exceed the performance criteria. For instance, if you have a value-engineering idea that reduces cost or time without compromising quality, please include it as part of your proposal. The City will consider such alternatives favorably during evaluation, provided they fulfill the intent. Trade-off Guidance: The evaluation will consider trade-offs between cost, schedule, and quality. For example, a proposal that offers a faster completion by using innovative precast elements may be rated higher in technical approach. Conversely, if a significantly lower-cost solution is proposed with a modest impact on aesthetics (but still meeting all requirements), the City may accept that trade-off. We ask bidders to identify any trade-offs they are making (e.g., higher upfront cost for lower lifecycle cost) and justify them. We have intentionally avoided mandating specific means and methods where not necessary, to allow offerors to utilize their expertise and creativity. Note: Any alternate solution should be clearly marked as an alternative and include supporting data to show it meets the required outcomes.â€ (This â€˜Aâ€™ excerpt is very welcoming to innovation: it explicitly says alternatives are encouraged and gives context on how those will be viewed (favorably if they meet criteria). It also signals that the RFP cares about performance over method in many areas and that creative trade-offs (like speed vs cost vs quality) will be considered â€“ essentially a best-value approach with openness to ideas. Bidders would feel empowered to propose their best ideas rather than just strictly follow a spec. This can lead to better project outcomes and potentially cost or time savings, as long as evaluation indeed rewards it.)

B (Good): The RFP allows some flexibility or hints at openness to innovation, but maybe not as overtly as an A. It might have some prescriptive elements but also include an â€œor equalâ€ clause for materials or a general statement like â€œequivalent alternatives will be considered.â€ Or the RFP might focus on outputs mostly, implicitly giving room for different means, but not explicitly inviting creativity â€“ bidders can infer they have flexibility because the RFP isnâ€™t micromanaging, though it doesnâ€™t celebrate innovation either. Trade-off guidance might be minimal; perhaps the RFP states the selection is best value, implying trade-offs are possible, but doesnâ€™t detail priorities. Still, nothing in the RFP forbids proposing a better idea, and a savvy bidder might include options or alternates. The evaluation criteria might even have a nod to â€œinnovationâ€ as a factor (e.g., 5% weight for unique value-added ideas). All requirements are necessary ones, with few â€œover-specifications.â€ Overall, a â€˜Bâ€™ RFP isnâ€™t hostile to innovation â€“ itâ€™s moderately flexible but not highly communicative about it. Example Excerpt: â€œThe design provided is conceptual. Bidders may refine the design as needed to meet the performance requirements. Materials or products specified by brand name should be considered as indicating the desired standard; substitutions of equal or better quality are permitted with approval. The City is interested in cost-effective solutions â€“ if you identify a change that could save cost or time, you may outline it as an option. However, the base proposal should be compliant with the stated requirements. The evaluation will primarily consider compliance and value, so innovative ideas that offer added value could positively impact your technical score.â€ (This â€˜Bâ€™ excerpt shows some flexibility: â€œequal or betterâ€ substitutions are allowed (common and important for fairness) and mentions cost-effective solutions and that you can outline them as an option. Itâ€™s a bit cautious (wants base proposal compliant, with alternates as optional) â€“ which is fine. It doesnâ€™t have as strong an encouraging tone as an A, but it does leave the door open. Bidders know they can propose alternates, though they might do so carefully. The mention that innovation could improve technical score is there but not very detailed. So, itâ€™s generally good â€“ not stifling innovation, but not a full-throated encouragement either.)

C (Average): The RFP is mostly prescriptive or standard, giving limited mention of flexibility. It likely sets out a solution in detail and expects bidders to adhere to it. It might not forbid alternatives, but it doesnâ€™t suggest them either. If a bidder had a different approach, they might be unsure if it would be welcome or cause their proposal to be deemed non-compliant. Perhaps the RFP has an â€œor equalâ€ clause in some sections, but other sections are very specific. Or it might allow alternates only in very trivial areas. Trade-off guidance is probably not there â€“ bidders are left guessing what the owner would value more if trade-offs are possible (e.g., is a shorter timeline worth a higher price? The RFP may not say). So, average here means the RFP sticks to requirements without discussing flexibility; bidders will likely just meet the spec rather than propose something novel, unless they take a risk. Example Excerpt: â€œThe scope and specifications outline the required solution in detail. All proposals should follow these specifications closely. If a bidder believes an alternative approach could be beneficial, they may present it, but must also provide a proposal that meets the RFP as written. The City has set performance requirements, and all proposals must demonstrate compliance. (No additional guidance on balancing cost vs quality is given; we expect bidders to deliver the best of both.)â€ (This â€˜Câ€™ excerpt is lukewarm. It basically says â€œfollow the specs.â€ It grudgingly allows that a bidder â€œmayâ€ present an alternative but only alongside a fully compliant offer â€“ this is a common approach in some procurements (alternates allowed only if a base compliant bid is given). It doesnâ€™t actively encourage it, nor does it indicate how such an alternate would be viewed (which might discourage bothering with it). Thereâ€™s no clear trade-off info â€“ they want the best of everything, which is nice but not realistic guidance. Bidders reading this will likely stick to exactly whatâ€™s asked for because the effort to propose an alternate might not be worth it if itâ€™s not clearly desired. Thatâ€™s average: itâ€™s not explicitly anti-innovation, but itâ€™s not facilitating it either.)

D (Poor): The RFP is highly rigid and leaves no room for innovation, possibly even disallowing any deviations. It might specify not just what to achieve but exactly how to do every step, possibly using outdated or overly specific methods, thereby preventing bidders from using newer techniques or efficiencies. It could say â€œno substitutionsâ€ for certain products without good reason (essentially locking into a specific vendorâ€™s product and stifling competitive alternatives). There is no indication that the owner would consider alternate proposals â€“ in fact, the language may warn that any deviation will render the proposal non-compliant. This rigidity can hurt the owner if an alternate could bring better value, but it often happens when specs are written in a very closed manner. Trade-offs are not addressed at all; the procurement likely treats all requirements as pass/fail with no flexibility â€“ e.g., they might be doing a lowest price technically acceptable approach where anything not exactly conforming is rejected. As a result, bidders will not propose anything creative and may even over-comply (doing exactly as told even if itâ€™s not optimal). Example Excerpt: â€œBidders must strictly adhere to the specifications and scope as written. No alternative proposals or exceptions will be accepted. The Agency requires the use of the specified construction techniques and materials â€“ for example, concrete mix design X and formwork method Y as described â€“ and will not consider variations. All proposals will be evaluated for compliance only. The focus is on meeting the RFP requirements at the lowest cost, not entertaining different approaches. Do not deviate from the plan.â€ (This â€˜Dâ€™ example clearly prohibits innovation: â€œno alternative proposals or exceptions.â€ It even prescribes specific methods and says will not consider variations, which slams the door on any contractor insight or improvement. The evaluation is compliance-only, implying any creativity is actually a risk to compliance. This is poor because it likely misses out on potential improvements contractors could offer, and it treats the RFP specs like gospel even if they might have inefficiencies. Vendors see this and will do exactly what is asked, nothing more â€“ possibly even if they know a better way, theyâ€™ll follow the spec to avoid being disqualified. The project might be fine, but it wonâ€™t get the benefit of industry expertise beyond the spec. It also indicates a likely trade-off stance: lowest cost for exactly that defined solution, period. Thatâ€™s not modern best practice except for commoditized work â€“ even then, some flexibility could help.)

F (Failing): The RFP is not only rigid but maybe impossibly prescriptive or contradictory in a way that prevents sensible trade-offs. It could require things that conflict, leaving no solution space, or demand an approach that is known to be suboptimal without allowing change. Or it could punish any deviation so severely that even asking a clarification about an alternate might get a bid thrown out. Another fail scenario: the RFPâ€™s priorities are completely unclear or misguided, so bidders cannot even discern how to craft a good solution â€“ for example, it might emphasize every aspect as most important, with no flexibility to prioritize, making trade-offs impossible (the infamous â€œfast, cheap, and high quality â€“ must have all threeâ€ with fixed constraints). Or the RFP might inadvertently discourage innovation by penalizing any differences (like an evaluation scheme that gives zero points for any feature not exactly as specified, even if better). Example Excerpt: â€œThe RFP details a step-by-step construction process that all bidders must follow exactly, with no room for adjustment, even if site conditions differ. For instance, it mandates Method A for soil stabilization, which may not even be feasible in all soil types, but alternative methods are explicitly disallowed. It also requires materials that are no longer manufactured, yet substitutions are forbidden â€“ leaving bidders in a quandary. The document makes every minor requirement critical, providing no indication of which aspects could be adjusted for a better overall outcome. Combined, these conditions make it impossible to propose a coherent solution without either breaking a rule or failing a requirement. Any attempt at proposing a different approach is grounds for rejection, and yet following the dictated approach might be unworkable. Bidders are essentially handcuffed to a potentially flawed plan with no discretion.â€ (This â€˜Fâ€™ scenario describes an RFP that is so prescriptive and inflexible that it may not even be executable (â€œmaterials no longer manufacturedâ€) and doesnâ€™t allow logical changes. Itâ€™s failing because it doesnâ€™t just stifle innovation â€“ it sabotages practicality. Bidders either have to violate the spec (and get rejected) or bid something they know wonâ€™t work just to stay compliant, hoping to sort it out after award (which is dangerous and could lead to change orders or failure). This is an extreme case, but a real example might be, say, an RFP that didnâ€™t update its specs in years and now requires outdated tech with a clause â€œno alternates,â€ leading to an untenable situation. Another failing aspect in the excerpt: it doesnâ€™t highlight priorities, meaning bidders canâ€™t even focus on whatâ€™s important, as everything is set in stone. In essence, the RFP has zero flexibility and even negative flexibility (contradictions), so it fails at guiding trade-offs or allowing any innovation. No responsible vendor would want to bid under those terms without significant contingencies or clarifications.)
...
GRADING SCALE:
A (Excellent): The RFP provides a comprehensive and clear list of all construction-related compliance requirements. It spells out mandatory contractor qualifications (e.g., â€œbidder must hold a State General Contractor license Class Aâ€), required bonds (bid bond with bid, performance and payment bonds of 100% each upon award), and insurance minimums. It details any safety requirements: for example, â€œContractor must comply with OSHA 1926; a site-specific safety plan must be submitted and approved before start.â€ If applicable, it notes prevailing wage or labor laws (â€œThis project is subject to federal prevailing wages â€“ see wage determination attached â€“ and certified payrolls will be required weeklyâ€). It mentions any specific permits or codes (â€œWork must comply with 2018 International Building Code and local amendments; contractor will be responsible for securing a building permit and inspectionsâ€). It might require proof of certain compliance capacities (like an Experience Modification Rate (EMR) for safety if they care, or that the contractor must follow a quality management program). All these requirements are typically placed in a section so bidders can easily find them, and possibly there are forms or certifications provided to fill (e.g., a Non-Collusion Affidavit, or a certification of compliance with employment law). In short, nothing in terms of legal or regulatory compliance is left out or ambiguous; bidders know exactly the playing field and requirements to adhere to. Example Excerpt: â€œContractor Licensing & Qualifications: Bidders must possess a valid [State] Contractorâ€™s License, Class B at the time of bid and throughout the project. Key supervisory personnel shall have OSHA 30-hour training. Safety Compliance: The Contractor must comply with all OSHA standards (29 CFR 1926). A site-specific Health and Safety Plan (HASP) is required within 10 days of award for approval. All workers must have at minimum OSHA 10-hour certification. Bonding & Insurance: A bid bond of 5% is required with the proposal. The awarded contractor must provide a Performance Bond and Payment Bond, each for 100% of contract value. Insurance requirements: $2M general liability per occurrence (City to be an additional insured), $1M auto, and statutory workersâ€™ comp. Labor Regulations: This project is subject to the Davis-Bacon Act â€“ prevailing wage rates are attached (Attachment D). Weekly certified payroll reports must be submitted. The contractor must also comply with the Cityâ€™s local hiring ordinance (see Attachment E). Environmental and Code Compliance: Contractor is responsible for obtaining a building permit and any trade permits; all work must conform to the 2018 IBC and local code amendments. An Erosion & Sediment Control Plan must be followed per state environmental regulations. Other Requirements: The contractor will need to furnish evidence of an Experience Modification Rate (EMR) of 1.2 or less for safety, or provide a safety action plan. All required compliance forms (non-collusion affidavit, EEO certification, etc.) are included in Appendix B and must be completed with the proposal.â€ (This â€˜Aâ€™ excerpt is lengthy, but thatâ€™s appropriate given the many compliance aspects. It clearly lists licensing, safety, bonding, insurance, labor law, permits/codes, and even specific forms. Bidders reading this know exactly what legal hoops and standards they must meet. Itâ€™s comprehensive â€“ no surprises later like â€œOh, we needed a particular bond or to pay certain wages we didnâ€™t account for.â€ This level of detail suggests the issuer is experienced and diligent, which attracts serious contractors and keeps the playing field level. It covers items typically found in public works projects in a clear way.)

B (Good): The RFP covers most major compliance requirements, but perhaps it misses one or two minor ones or isnâ€™t as detailed in explanation. For example, it might list the need for license and bonds and refer to prevailing wages, but not attach the wage schedule (assuming bidders will find it themselves, which is okay but not as convenient). Or it states OSHA compliance generally but doesnâ€™t specifically call for a safety plan submittal (perhaps itâ€™s implied but not explicit). It might mention insurance but not the exact amounts (maybe referencing a standard insurance requirement in an appendix that bidders are assumed to know). Nonetheless, all key topics are at least mentioned: e.g., it says â€œmust comply with all applicable laws and codes, including building codes, safety regulations, labor lawsâ€ â€“ which covers it broadly if not specifically. A diligent bidder can infer what to do, but a newcomer might have to clarify some details. Still, nothing critical is totally omitted â€“ itâ€™s just not as spoon-fed or perfectly organized as an A. Example Excerpt: â€œBidders shall have a valid construction contractor license and be bondable. A bid guaranty (bid bond) of 5% is required. The successful bidder must furnish performance and payment bonds. Contractors must carry adequate insurance (per City requirements) and comply with all safety and labor regulations. This project falls under prevailing wage laws; current wage rates will apply (state prevailing wage for building construction). Contractor is responsible for obtaining necessary permits and following applicable building codes. The Cityâ€™s standard contract in Appendix C further enumerates these compliance requirements.â€ (This â€˜Bâ€™ example covers the bases but not in extreme detail. It assumes a bit that bidders know â€œper City requirementsâ€ for insurance (likely listed in standard contract). It references prevailing wage but doesnâ€™t provide them explicitly in the text (assuming bidders know or will find state rates). It does say comply with safety and labor regulations generally â€“ it doesnâ€™t specify OSHA or details like requiring a safety plan, but one would assume it. It points to the standard contract for more, which is fine, but an A might have pulled more of those highlights into the RFP body. Overall, a competent bidder sees nothing alarming missing, but they might have to dig in the appendix or know standard practice to get details. Itâ€™s still good as it addresses the main topics with a reasonable breadth.)

C (Average): The RFP mentions some compliance requirements but misses several or is too generic. It might only briefly mention that the contractor must obey laws and obtain permits, without calling out specifics like prevailing wage or specific bonds. Possibly itâ€™s written for a private project where some of these (like prevailing wage) donâ€™t apply, but it still should mention safety and insurance, etc. If itâ€™s silent on needed bonds or insurance, thatâ€™s a gap but maybe those are industry standard enough that bidders expect them (still, not listing them can cause uneven assumptions). It might not state a license requirement â€“ maybe taken for granted or in a separate advertisement but not in the RFP text. It could assume the contractor knows to follow OSHA and building codes, and thus not explicitly say it â€“ which most will, but not stating can be risky if a bidder tries to cut corners. Essentially, average means some important compliance items are not explicitly addressed, though likely required by default. Bidders might need to ask or assume the strictest case to be safe. Example Excerpt: â€œContractor shall comply with all applicable laws and regulations. It is expected that the contractor maintains proper licensing and insurance. The contractor will be responsible for jobsite safety and adherence to applicable OSHA standards. All necessary permits for construction must be obtained by the contractor or through coordination with the owner. The contractor must adhere to all state and local labor laws.â€ (This â€˜Câ€™ excerpt is very generic. It says the right things in principle â€“ obey laws, have licensing, follow OSHA, get permits â€“ but it doesnâ€™t mention bonds at all (maybe they forgot or are assuming a low value job where bonds might not be needed; if bonds are needed, thatâ€™s a miss). It doesnâ€™t specify insurance coverage amounts â€“ just says maintain insurance (bidders might wonder how much). It doesnâ€™t mention prevailing wage or specific labor compliance if relevant (just all labor laws, which is broad). Itâ€™s not that the contractor can ignore something â€“ â€œall applicable lawsâ€ technically covers everything including wages and safety â€“ but the RFP isnâ€™t helping the bidder by highlighting key compliance tasks or costs. A bidder may need to verify if prevailing wage applies or if a bond is needed by looking at external rules or asking. So itâ€™s average â€“ covering itself legally but not being explicit or thorough. Some contractors might overlook a requirement not stated and get in trouble later, which is why explicitness is better.)

D (Poor): The RFP fails to mention several critical compliance requirements, potentially leading to serious oversights. For example, it might not mention the need for a performance bond or insurance at all, leaving bidders not including those costs or not even having the capability lined up. Or it could omit the fact that itâ€™s a public project requiring prevailing wages â€“ a bidder might assume standard wages and bid lower, only to find out later they must pay higher wages (which could cause financial issues or legal issues). It might also fail to address safety expectations â€“ not that the contractor wonâ€™t be legally obligated anyway, but not stating it could mean the owner hasnâ€™t set any additional safety requirements (like submittal of plans or training) and isnâ€™t emphasizing safety culture. If licensing isnâ€™t mentioned, perhaps an unlicensed or improperly licensed firm might bid (in some jurisdictions bids by unlicensed contractors are void, but if not clearly stated, it could slip through). A poor RFP might assume contractors know all these by default, but that assumption can be dangerous especially if out-of-state bidders or newcomers are involved. Example Excerpt: â€œThe project will be executed under the terms of the contract. (No specific mention of bonds, insurance, licensing, safety, or labor standards is provided in the RFP document.)â€ Or, another form: â€œGeneral conditions will apply.â€ with no details. (This â€˜Dâ€™ scenario illustrates an RFP that basically punts on all these details, maybe thinking the standard contract covers it â€“ but if bidders donâ€™t see the standard contract until after bidding, they might not realize the requirements. For instance, if a small contractor bids not realizing a $5M insurance policy is required, they might not even be able to get that. Or if no mention of prevailing wage, a bidder might budget $20/hr for labor when actually they needed $40/hr rates â€“ a huge miss. The RFPâ€™s silence is risky. Also, not highlighting safety or permit responsibilities could lead to confusion on roles. Perhaps the RFP is extremely short or a private owner who didnâ€™t know to include these. In any event, itâ€™s poor because key compliance issues arenâ€™t proactively addressed. Bidders might need to guess or will ask a lot of clarification questions. It can lead to uneven bids or post-award conflicts when the winner says â€œI didnâ€™t include X because it wasnâ€™t in the RFP.â€)

F (Failing): The RFP not only omits compliance requirements but possibly conveys incorrect or illegal instructions, or exhibits ignorance of standard legal requirements. For example, an RFP that tells bidders they donâ€™t need to follow a certain law when in fact they do (like â€œwe donâ€™t require workersâ€™ comp insuranceâ€ â€“ which might be illegal to waive). Or one that sets contradictory compliance obligations (like imposing two different sets of wage rates without clarity). Or perhaps itâ€™s a public project but the RFP fails to include statutorily mandated provisions (EEO, etc.), which could invalidate the procurement. If an RFP is missing all such provisions, it could be considered non-compliant with procurement rules itself. Another failing case: if an RFP required something not permissible (like asking bidders to pay a kickback or something unethical â€“ hopefully rare). Essentially, failing is when compliance is so mishandled that it may lead to legal challenges or project stoppage. Example Excerpt: â€œ(No mention of prevailing wage on a project clearly subject to it, no mention of bonds or safety, etc., anywhere in the documents.) After award, the owner adds these requirements.â€ Or an explicit wrong thing: â€œContractor is not required to carry workersâ€™ compensation insurance for this projectâ€ (when law says they must). (Failing case is often an omission â€“ similar to poor â€“ but to an extreme degree, such as a federally-funded project with none of the required federal clauses in the RFP. This could result in protests or losing funding. Another scenario: the RFP might be so negligent that, say, it awards to an unlicensed contractor due to not screening that, leading to legal voiding of the contract. While it might not appear in the RFP text, failing to state such prerequisites could cause that outcome. For the bidder, a failing RFP sets them up for potential legal issues â€“ if they follow the RFP and the RFP was non-compliant with the law, itâ€™s a mess. Thus, failing grade is warranted when the RFPâ€™s handling of compliance is fundamentally flawed or missing to the point of jeopardizing the project or fairness.)
...
GRADING SCALE:
A (Excellent): The RFP provides very clear, concise, and well-organized submission instructions. It specifies exactly what the proposal should contain and how it should be structured. For example, it might include an outline: â€œSection 1: Executive Summary; Section 2: Technical Approach; Section 3: Project Team; Section 4: Past Experience; Section 5: Price Proposal (separate).â€ It states the format (e.g., PDF or hard copy requirements), page limits or formatting rules if any (font size, page count), and how to label sections. It also gives the submission procedure: e.g., â€œUpload to procurement portal X by 5:00 PM on July 1, 2026â€ or â€œSubmit one electronic PDF and two hard copies to the addressâ€¦â€. There is no ambiguity about deadlines, addresses, or required documents (forms, cover letters, etc., are all enumerated). Essentially, a bidder reading it knows exactly how to prepare and deliver their proposal. Example Excerpt: â€œProposal Format: Offerors shall organize their proposals into the following sections: 1) Cover Letter (1 page); 2) Technical Proposal (max 30 pages) covering methodology, schedule, and staffing; 3) Experience and Qualifications (max 20 pages, including resumes and project examples); 4) Price Proposal (use the Price Form in Appendix D, submitted separately). Submission: Proposals are due August 15, 2026 at 2:00 PM EST. Submit via the Cityâ€™s eProcurement portal (link provided). File Format: PDF, with filenames including the bidder name. No email or fax submissions will be accepted. Additional Requirements: Include a signed copy of the Attachment A (Non-Collusion Affidavit) and Attachment B (Bid Bond Certification) with your proposal.â€ (This â€˜Aâ€™ instructions excerpt is thorough and specific. A bidder following this knows precisely how to format their response and wonâ€™t be tripped up by missing forms or wrong format. Itâ€™s also concise and not overburdened with trivial requirements â€“ just the essentials clearly stated.)

B (Good): The RFPâ€™s submission instructions are generally clear, but may have minor issues. Perhaps everything important is there (what to include and how to submit), but one or two details could be clearer â€“ for instance, it might not specify a preferred format for certain sections, or it may require an outdated practice like multiple hard copies unnecessarily. Overall, though, a diligent bidder can figure out whatâ€™s needed without having to ask questions. The instructions might be somewhat lengthy or bureaucratic but are still understandable. Example Excerpt: â€œProposal Submission: Email your proposal to procurement@agency.org by 5:00 PM September 10. The proposal should be in PDF format. Include sections for your approach, team, past performance, and a separate pricing section. Hard copy is not required, but if provided, submit 1 original and 2 copies. All proposals must reference RFP #123 in the subject line. Page limit: 50 pages not including appendices.â€ (This â€˜Bâ€™ example is mostly clear: it tells where and when to submit, and generally what to include. Itâ€™s a bit mixed between email and optional hard copies, which could be simplified, but itâ€™s not confusing to the point of causing errors. It might not provide a strict section outline, but the expectation of sections is hinted. Itâ€™s good, if not perfect.)

C (Average): The RFPâ€™s instructions exist but lack clarity or completeness in places. Bidders might be left unsure about some formatting details or whether certain documents are required. Perhaps the RFP says â€œsubmit a complete proposalâ€ without outlining the sections, or it gives contradictory instructions (e.g., one part says submit via email, another part says via portal). Maybe the due date or time zone isnâ€™t clearly stated (leading to potential confusion). In general, the basics can be deduced, but thereâ€™s room for misinterpretation. Example Excerpt: â€œProposals are due Friday, 5 PM. Submit your proposal in a sealed package to our office. The proposal should contain all necessary information for evaluation. You may also email a copy. Include any relevant experience and pricing info.â€ (This â€˜Câ€™ instruction is problematic: it says â€œFriday, 5 PMâ€ but not the date or time zone, and mentions both sealed package and email without clarity â€“ do both need to be done? It doesnâ€™t outline the content structure, just says â€œall necessary information,â€ which is pretty open-ended. Bidders could interpret differently what to include. There is a risk of inconsistency in responses or disqualification for not including something the issuer expected but didnâ€™t explicitly ask for.)

D (Poor): The submission instructions are confusing, contradictory, or incomplete to a serious degree. Bidders likely will be uncertain how to comply. For example, the RFP might fail to specify where or how to submit (no email or address given), or might include conflicting directions (e.g., â€œSubmit 3 copiesâ€ in one section and â€œSubmit 5 copiesâ€ in another). It could also impose overly complicated format rules that are hard to follow and seemingly unnecessary, which can deter bidders. Example Excerpt: â€œProposal Instructions: Submit your proposal soon. Be sure to include everything. Format isnâ€™t specified, but compliance with all requirements is mandatory. One section of the RFP states you need to mail hard copies, another section says electronic submission through a portal â€“ use your best judgment. Failure to follow the exact (though unspecified) format may result in disqualification.â€ (This â€˜Dâ€™ example illustrates a muddled instruction set. Bidders would be scratching their heads: When is it due? Where exactly to send it? What does â€œeverythingâ€ include? The mention of disqualification without clear format guidance is especially problematic. It shows the RFP issuer demands compliance without providing the roadmap â€“ a major quality issue.) Alternatively, a real-world style example of burdensome instructions that lean D: â€œStrict conformance to the specified proposal format is essential. Proposals shall be submitted in three-ring binders with each section tabbed as per the outline. Provide one original with wet signatures, three copies, and a digital version on CD. Use 12-pt Times New Roman font, single-spaced, with 1â€ margins. Any deviation may result in rejection.â€ This, drawn from actual RFPs, is extremely prescriptive. While clear, such micromanagement can signal bureaucracy over substance and discourage bidders (especially requiring physical copies in the digital age), thus weâ€™d rate it poor in quality of approach.

F (Failing): The RFP provides no clear submission instructions, or instructions that are so erroneous that bidders are likely to miss the mark. For instance, an â€˜Fâ€™ case is an RFP that does not state a due date/time or method at all â€“ leaving bidders guessing how to deliver their proposal. Another failing scenario is if the instructions are wrong (e.g., list an incorrect email or a past date). Or perhaps the RFP mixes up instructions from another procurement (copy-paste error) that donâ€™t apply, causing maximum confusion. Example Excerpt: â€œProposals should be sent to us by the deadline. Good luck.â€ Or an even worse real example: an RFP that only says, â€œPlease submit a proposal.â€ with no further info. Another failing scenario: â€œSubmit your proposal online.â€ (with no link or portal specified, no deadline given). (In these cases, the issuer has essentially not given the bidders a clear way to comply. Itâ€™s as if the RFP forgot to include the instructions section. Such an omission or mistake is a critical failure â€“ proposals might not even reach the right place or time, or could be deemed late or non-compliant through no fault of the bidders. A competent procurement process cannot function with instructions at this level.)
...
GRADING SCALE:
A (Excellent): The RFP provides a very clear and structured communication plan. It explicitly states the deadline for submitting questions (e.g., â€œQuestions must be submitted via email to the Procurement Officer by July 10, 2025, 5:00 PM.â€) and the format (perhaps a specific subject line or form). It names the single point-of-contact (with email/phone) for all inquiries and instructs that no other communication is allowed (preventing off-record Q&A). It commits to a date by which written responses to all questions will be issued to all registered bidders, often in the form of an addendum or Q&A document, ensuring everyone gets the same information. The RFP also encourages questions (like stating that bidders should seek clarification on any uncertainties). Additionally, it outlines the addenda procedure: for example, â€œAny changes to the RFP will be issued via formal addendum on the project website. Bidders must acknowledge all addenda in their proposal.â€ It may even detail a pre-proposal conference or site visit if applicable and how clarifications from that will be handled (usually via written summary afterward). This level of clarity shows the issuer is organized and committed to transparency. Example Excerpt: â€œCommunications Protocol: All questions or requests for clarification regarding this RFP shall be submitted in writing via email to Jane Doe, Procurement Officer (jdoe@city.org) no later than March 1, 2026 at 4:00 PM. Questions will be answered via a written Q&A Addendum. The City will compile all questions received and issue a single Addendum (Addendum #1) by March 8, 2026, to all known RFP holders via the Cityâ€™s e-bidding portal. Only answers provided in writing via such addendum are official and binding. Bidders are not to contact any other City staff or stakeholders regarding this RFP, to ensure fairness. In addition, if any changes to the RFP requirements or timeline are needed, the City will issue subsequent addenda. Bidders should check the portal for any updates and must acknowledge each addendum in their proposal cover letter. Pre-Proposal Meeting: A non-mandatory site visit will occur Feb 20, 2026; any clarifications arising will be posted in the Q&A addendum as well. We encourage bidders to ask questions â€“ if any requirement is unclear, please use the Q&A process so we can clarify for all parties.â€ (This â€˜Aâ€™ excerpt comprehensively lays out how communication happens. It has deadlines, contact person, method (email), commitment to written answers for everyone, rules against unauthorized contact, and the mechanism of addenda. A bidder reading this knows exactly how to get info and trust they wonâ€™t miss anything. Itâ€™s an exemplary approach that fosters fairness and minimizes confusion.)

B (Good): The RFP does define a Q&A and addenda process, but perhaps with slightly less detail or a minor gap. For instance, it might list a contact and deadline for questions, but not explicitly say how answers will be provided â€“ though one can assume an addendum will be issued, it might not say â€œwe will issue an addendum by X dateâ€. Or vice versa, it might promise an addendum but didnâ€™t clearly set a cutoff for questions (leading to potential late questions). It might also omit the instruction about not contacting others, which is a small lapse in ensuring all communication is through one channel. Nonetheless, the essentials are there: bidders know who to ask and roughly when, and that answers will be shared. Addenda are mentioned as the vehicle for changes. Any omissions are likely ones an experienced bidder could infer or wouldnâ€™t significantly harm the process. Example Excerpt: â€œDirect all inquiries to Procurement@agency.gov. Questions should be submitted ASAP, preferably by April 5, 2025. The Agency will issue clarifications to all participants â€“ check the project website for updates (addenda). It is the bidderâ€™s responsibility to obtain all addenda. Do not rely on oral statements; only written responses are official. Changes to the RFP, if any, will be posted as addenda.â€ (This â€˜Bâ€™ example conveys the general idea: email questions by a date, answers via website postings. Itâ€™s mostly good, though it uses language like â€œpreferably by April 5â€ rather than a hard deadline â€“ slightly loose, but still gives a target. It also places onus on bidders to check the site rather than explicitly emailing them, which is okay but a bit less hand-holding. Importantly, it mentions written vs oral and addenda usage. It could improve by confirming the exact addendum issue date or instructing no contact with others, but overall itâ€™s a clear process. Most bidders would find this acceptable and understand how to proceed.)

C (Average): The RFP includes some information about Q&A or addenda, but itâ€™s incomplete or a bit unclear. Possibly it says â€œquestions may be submittedâ€ but does not give a deadline, leaving bidders unsure of the cut-off (they might assume or have to ask when the last day is). Or it provides a contact person but maybe no explicit instruction that thatâ€™s the only contact (leaving ambiguity whether you could call someone else). It might not explicitly promise to share answers with all â€“ maybe it implies it but doesnâ€™t state the method (some bidders might worry if they donâ€™t hear an answer directly). The addendum process might be vaguely referenced like â€œif any changes, we will issue addendaâ€ but without guidance on how those will be communicated (email, website?). Essentially, an average scenario covers the basics in passing but lacks detail. Bidders might have to assume standard practices or ask a meta-question about the Q&A process itself. Example Excerpt: â€œAny questions regarding this RFP should be directed to the Project Manager. Answers to questions that impact the proposal may be shared with other bidders. The City reserves the right to issue addenda to modify this RFP if needed.â€ (This â€˜Câ€™ blurb is short and not very specific. It doesnâ€™t give a deadline or format for questions, just says contact the Project Manager â€“ which at least is a single point, but by name/email would be better. It says answers â€œmay be sharedâ€ which is weak â€“ it should be will be shared, and how? It mentions addenda if needed but no instruction on how theyâ€™ll be delivered or acknowledged. Bidders reading this might be unsure: â€œUntil when can I ask? Will I get answers individually or only via some posting? Who exactly is the Project Manager contact info?â€ They might have to dig for the contact details elsewhere in the RFP. This is average â€“ workable if you assume typical procedures, but not explicitly laid out.)

D (Poor): The RFPâ€™s guidance on communications is minimal, confusing, or flawed. Perhaps it fails to mention any Q&A process at all â€“ bidders donâ€™t know if they are allowed to ask questions or how. Or it mentions something but in a confusing way, e.g., multiple contacts or channels (leading to risk some bidders get info others donâ€™t). It might allow questions but very late or not plan to answer them properly. A poor case could also be if the RFP encourages contacting various people or does not warn against it â€“ possibly resulting in unequal information distribution or even violation of procurement rules. Addenda might not be mentioned, or the RFP might rely on bidders to figure out changes on their own. In any event, a â€˜Dâ€™ means likely some bidders will be left in the dark or get different answers (lack of a controlled process). Example Excerpt: â€œQuestions can be phoned in to our office or emailed. Weâ€™ll try to answer what we can. If any big changes come up, we might update the RFP document on the website without notice. Itâ€™s up to bidders to stay informed. Also, technical questions can be directed to the engineering consultant, while contractual questions go to purchasing.â€ (This â€˜Dâ€™ scenario is quite disorganized: multiple channels (phone, email, different contacts for tech vs contract) â€“ which increases the chance of inconsistent answers or some bidders getting info others donâ€™t. Thereâ€™s no deadline given â€“ implying you could call up until the due date? Thatâ€™s problematic. â€œMight update without noticeâ€ is really bad â€“ bidders could miss changes. It basically puts burden on bidders to keep checking if something changed, with no formal addendum distribution. Also splitting questions by topic to different people can cause confusion or overlaps. This would be a poorly managed communication plan, likely resulting in unfairness or frustration.)

F (Failing): The RFP provides no formal communication mechanism, or actively discourages needed clarification, resulting in a chaotic or opaque situation. For example, an RFP that is silent on Q&A means bidders have no official way to resolve doubts â€“ they either guess or possibly try back-channel contacts, which can lead to misinformation or unfair advantages. Or an RFP could explicitly forbid asking questions (â€œNo questions will be answeredâ€), which is a terrible practice because it means any ambiguity stays that way, and bidders might interpret requirements differently (leading to apples-to-oranges proposals or lots of errors). Another failing case is if changes are made last-minute without proper notification, or answers are given only to some and not all, undermining fairness. Essentially, failing means the communication process breaks the fundamental procurement principle of equal information. Example Excerpt: â€œThe City will not entertain any questions or requests for clarification about this RFP. Bidders must rely solely on the information contained herein. Any updates, if made, will not necessarily be communicated to all bidders. It is expected that bidders understand the requirements without further explanation.â€ (This extreme â€˜Fâ€™ example explicitly shuts down communication. If the RFP document is less than perfectly clear (almost always the case), this stance means misunderstandings canâ€™t be clarified. Itâ€™s likely to result in either no bids or highly cautious bids with many assumptions. Also saying updates â€œwill not necessarily be communicated to allâ€ is essentially admitting an unfair practice â€“ maybe theyâ€™d tell whoever asks but not proactively others, which is a big no-no. Such an RFP might be non-compliant with procurement regulations or at least highly risky for the owner. In summary, an RFP that either forbids Q&A or doesnâ€™t have a mechanism for it is failing this criterion â€“ it signals a non-transparent or incompetent process. Bidders could either all interpret things differently or waste effort on flawed proposals, and the owner might end up with a failed competition or legal challenges.)
...
- Do NOT mention any numeric scores or percentages (like "85/100" or "85%") in text fields â€” only use letter grades (A+, A, A-, B+, B, B-, C+, C, C-, D+, D, D-, F)
...
1. RFP DOCUMENTS (Source of Truth) â€” appearing first in the content
2. VENDOR RESPONSE DOCUMENTS â€” appearing after the "--- VENDOR RESPONSE DOCUMENTS BELOW ---" separator
...
A (Excellent): Fully compliant and complete. The proposal follows every instruction to the letter and includes all required information/documentation. A compliance checklist was clearly used â€” nothing is missing or out of place. All required sections (safety plan, QC plan, schedule, pricing, resumes, bonding letter, insurance certs, past projects) are included in the exact order and naming convention requested, with all forms signed and properly tabbed. Submission rules (page limits, font size, cover sheets, portal uploads) are followed precisely.
...
CRITICAL RULES:
- Do NOT mention any numeric scores or percentages (like "85/100" or "85%") â€” only use letter grades (A+, A, A-, B+, etc.)
- Your assessment must reflect the ACTUAL quality of this specific proposal â€” do NOT default to any particular grade range
...
OUTPUT FORMAT (JSON only):
{
  ...
  "evaluatorInsight": {
    "overallImpression": "What an experienced evaluator would think upon reading this proposal â€” 2-3 sentences",
    "competitivePosition": "How this proposal likely stacks up against typical competitors â€” 1-2 sentences",
 ...
 * @param {Array} fileUris - Array of {fileUri, mimeType} objects
 * @param {Object} genAIClient - Google GenAI client
 * @param {Object|null} reporter - Live-progress reporter (gradeRuns/{runId})
 * @return {Promise<Object>} Complete grading result
 */
const gradeRfpMultiPass = async (fileUris, genAIClient, reporter) => {
  ...
  for (let i = 0; i < variableEntries.length; i += batchSize) {
    ...
    // Live progress: this batch's grades land on the page now.
    if (reporter) {
      const variables = {};
      batch.forEach(([key, config], idx) => {
        const r = batchResults[idx] || {};
        variables[key] = {
          name: config.name,
          grade: r.grade || "â€”",
          note: firstSentence(r.reasoning),
        };
      });
      await reporter.report({stages: {variables}});
    }
  ...
  if (reporter) {
    await reporter.report({stages: {specials: {
      killerClauses: {
        found: !!killerClauses.found,
        count: killerClauses.count || 0,
        severity: killerClauses.severity || "none",
      },
      ambiguity: {
        index: ambiguity.index || 0,
        category: ambiguity.category || "unknown",
        totalIssues: ambiguity.totalIssues || 0,
      },
    }}});
  }
  ...
  if (reporter) {
    const rec = (perspectives.bidderPerspective || {}).bidRecommendation || {};
    const pwin = (perspectives.bidderPerspective || {}).pWinRate || {};
    await reporter.report({verdict: {
      grade: aggregated.grade,
      summary: firstSentence(perspectives.executiveSummary),
      recommendation: rec.action || "review",
      pwinMin: pwin.min ?? null,
      pwinMax: pwin.max ?? null,
    }});
  }
  ...
  try {
    ...
    return {
      ...
      evaluatorInsight: {
        ...
        competitivePosition: "Unable to assess â€” synthesis failed.",
      ...
      <tr>
        <td style="width: 25%; padding: 14px 10px; text-align: center;
          ...
          <div style="font-family: 'Helvetica Neue', Arial, sans-serif;
            ...
            ${bidder.pWinRate?.min || "?"}â€“${bidder.pWinRate?.max || "?"}%</div>
        </td>
        <td style="width: 25%; padding: 14px 10px; text-align: center;
                   border-right: 1px solid #000000;">
          <div style="font-family: 'Helvetica Neue', Arial, sans-serif;
...
const generateRFPGradePDF = (result) => {
  return new Promise((resolve, reject) => {
    ...
    const gridData = [
      ["PWIN RATE",
        `${bidder.pWinRate?.min || "?"}â€“${bidder.pWinRate?.max || "?"}%`,
        false],
      ["COST OF WASTE",
    ...
    if (kc.analysisError) {
      ...
      lbl(`${kc.count} KILLER CLAUSE(S) â€” SEVERITY: ${
    ...
    imps.forEach((imp) => {
      doc.font(FB).fontSize(10).fillColor(BK);
      doc.text(`â€¢ ${imp.title || imp.description || ""}`,
    ...
    if (issuer.topPriorities && issuer.topPriorities.length > 0) {
      ...
      issuer.topPriorities.forEach((p) => {
        doc.font(FB).fontSize(10).fillColor(BK);
        doc.text(`â€¢ ${p}`, lX, lY, {width: colW, lineGap: 4});
        lY = doc.y + 4;
      });
    }
    lY += 12;
    ...
    qs.forEach((q) => {
      doc.font(FB).fontSize(10).fillColor(BK);
      doc.text(`â€¢ ${q}`, rX, rY, {width: colW, lineGap: 4});
      rY = doc.y + 6;
    });
    ...
    rY += 12;
    doc.font(FH).fontSize(10).fillColor(BK);
    doc.text(`PWIN RATE: ${bidder.pWinRate?.min || "?"}â€“${
...
const gradeRfpHandler = async (req, res) => {
  ...
  const {email, mode, rfpFiles, responseFiles, runId} = req.body;
  let reporter = null;
  ...
  try {
    ...
    // the opportunity feed. Fail closed: no counter, no run.
    try {
      await assertDailyCap();
    } catch (capError) {
      if (capError.code === "over_capacity") {
        return res.status(429).json({
          error: "The grader is at capacity for today. " +
            "Email daniel@propagent.ai and we'll run yours by hand.",
        });
      }
      logger.error("Daily cap check failed:", capError);
      return res.status(503).json({
        error: "Grading is temporarily unavailable. Please try again soon.",
      });
    }

    // Live progress doc for the homepage lab (RFP mode only).
    if (mode === "rfp") {
      reporter = createRunReporter(runId);
      if (reporter) {
        await reporter.report({
          status: "processing",
          mode: mode,
          fileName: String(rfpFiles[0].name || "document").slice(0, 120),
          createdAt: admin.firestore.FieldValue.serverTimestamp(),
          stages: {intake: {done: false}},
        });
      }
    }
    ...
    // Files are converted and uploaded â€” intake is done.
    if (reporter) {
      await reporter.report({stages: {intake: {done: true}}});
    }
    ...
    if (mode === "rfp") {
      ...
      gradingResult = await gradeRfpMultiPass(
          uploadedFiles.rfp, genAI, reporter);
    } else {
    ...
    if (reporter) {
      await reporter.report({status: "complete"});
    }
    ...
    if (reporter) {
      await reporter.report({
        status: "error",
        error: "The analysis hit a snag. Your document was not retained; " +
          "please try again.",
      });
    }
...
const gradeRfp = async (req, res) => {
  // Set CORS headers (allowlisted origins only; non-browser callers
